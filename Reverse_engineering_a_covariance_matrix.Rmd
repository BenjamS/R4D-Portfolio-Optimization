...
Agricultural research for development (AR4D) institutions tend to give careful consideration to the impact assessment of individual projects in their portfolio, but very little, if any, consideration to the tradeoffs and synergies that may arise between projects. Project synergies can enhance portfolio impact while tradeoffs can effectively annul it. A clear understanding of these tradeoffs and synergies is therefore essential to the optimal leveraging of investments.

In statistical parlance, synergies and tradeoffs are known as positive and negative covariances, respectively. In the AR4D context, there is typically no data by which to calculate project covariances. AR4D institutions can usually draw on a wealth of domain expertise to shed light on these synergies and tradeoffs, but it is not clear how such ad hoc assessments can be quantified. There are also inevitably gaps where domain experts are unable or hesitant to venture an estimate. For example: What is the synergy or tradeoff between a heat tolerant bean project and a digital agriculture project?

Here I propose a method for "reverse engineering" project covariances from domain knowledge in such minimum data contexts. Specifically, I propose a survey whereby each project's correlation with the institution's strategic objectives (SOs) is elicited from domain experts. Drawing on principal components analysis (PCA), I show how an approximate project covariance matrix can be deduced as the outer product of these crowdsourced project-SO correlations.

In the process, it turns out that the project covariance problem is mathematically dual to another, parallel problem: AR4D institutions tend to give careful consideration to the formulation of their strategic SOs, but very little, if any, consideration to the tradeoffs and synergies that may arise between them. Again, this issue is critical for optimal leveraging of resources. How do investments under an Economic Growth SO, for example, capitalize upon or work against investments under a Nutritional Security SO? The method proposed here can be used to reverse engineer both project and SO synergies/tradeoffs.

I first provide a proof of concept using data from financial markets. I then walk through an illustrative example of how the method might be applied in the AR4D context. I conclude by sketching out a couple of potential directions for future exploration.
...
# The signals correlation matrix and "signal ROI"
...
The foregoing implies that it is possible to work backwards from the orthogonally rotated loadings $L_{\circlearrowleft}$ to arrive at an approximate data correlation matrix. In other words, instead of starting with
a correlation matrix and working towards a definition of the principal components, one starts from well defined principal components, and works towards a definition of the correlation matrix.

In the AR4D context, an institution's SOs are comparable to a set of principal components describing 90% of the problem space that is of interest to the institution. Given a portfolio of projects, a survey of domain experts and/or stakeholders could be conducted to determine how correlated each project is with each SO. These correlations may then be interpreted as orthogonally (but not necessarily varimax) rotated loadings $L_{\circlearrowleft}$ corresponding to an unobserved dataset measuring project impact.^[It is important to have a clear notion of this unobserved dataset. When two projects are said to be correlated, what exactly about the two projects is correlated? In many cases, project return on investment (ROI) may be the variable of interest. But other measures might also be considered, such as net present value or scaleability.] The project impact correlation matrix can then be calculated from this information via equation \ref{eq:cormat_from_Lrot}, and the SO correlation matrix can be calculated via equation \ref{eq:crowdsource_SOcormat}.
...
A list of hypothetical AR4D projects is presented in table \ref{tab:exampLoadings}. The projects are loosely grouped into four categories to facilitate interpretation, but there is no strict rule followed, and clearly some overlap, in the grouping.
...
```{r, fig.show='hold', fig.width=5, fig.height=3, fig.align='center', fig.cap="\\label{fig:corrXS_tratVarty}Hypothetical example of crowdsourced, orthogonally rotated trait-variety correlations.", echo=F}
#---------------------------------------------
n_varieties <- 10
...
gg_covmat_varty <- plot_covmat(cormat_XX_derived, fig_title, round_to = 2, graph_on = F, legend_position = "none")
...
gg_covmat_trait <- plot_covmat(cormat_SS_derived, fig_title, round_to = 2, graph_on = F, legend_position = "right")
...
On the other hand, it _is_ possible to optimize criteria weights, i.e. the weight or emphasis given to each SO by the institution and/or stakeholders. Such weights are typically assigned in a highly subjective and ad hoc manner. The method presented here suggests a more rigorous, objective approach. Unlike the reverse engineered project correlation matrix, the reverse engineered SO corrrelation matrix (equation \ref{eq:crowdsource_SOcormat}) is invertible, and can thus be used in an optimization problem such as that which is stated above. In the formulation of the objective function $E[U]$, project ROI is replaced with SO ROI, deduced from the crowdsourced loadings via equations \ref{eq:deducePcrowd} and \ref{eq:sigs_def}. Again, care must be taken to ensure positive weights^[One simple (and perhaps facile) way to ensure positive weights is to replace $\mathbf{w}$ with $\ln(\mathbf{w})$.] and an accurate representation of the benefits of marginal increases in weights.
