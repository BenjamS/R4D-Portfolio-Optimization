...
By definition, project NPV appreciates over time at the quarterly discount rate $r$. Henceforth, then, the expected quarterly growth rate $m$ is assumed to be $r$.
<!-- Conversely put, a project growth rate different from $r$ implies that the project NPV is inaccurately calculated. Therefore, by assuming that project NPV is accurately calculated, one effectively assumes that the expected arithmetic return $m$ equals $r$. -->
...
\begin{equation}
\begin{split}
E \left[\ln \left (\frac{x(\tau)}{x(\hat{t})} \right) \right] \biggr|_{t = \hat{t}} &= \left(m + \frac{s^2}{2} \right) \tau \\
Var \left[\ln \left( \frac{x(\tau)}{x(\hat{t})} \right) \right] \biggr|_{t = \hat{t}} &= s^2 \tau
\end{split}
\label{eq:EVarlogRet}
\end{equation}
...
## Expected value, variance, covariance, and "covariation" of project NPVs

Since this is an article about portfolios, project specific parameters such as $s$, $\tau$, and $T$ henceforth carry the project subscript $k$, with $k \in (1, n)$, where $n$ is the number of projects in the portfolio. Under the assumption that project NPV follows a gBm, the expected value and variance of the $k^{th}$ project NPV are
<!-- The non-infinitessimal time span $T_k$ henceforth refers specifically to the project time horizon. -->
<!-- at the end of the funding cycle ($x_k(T_k)$) -->

\begin{equation}
E[x_k(T_k)]\bigr|_{t = \hat{t}} = \mu_k = x_k(\hat{t}) e^{r \tau_k}
\label{eq:ExTraw}
\end{equation}
...
\begin{equation}
Var[x_k(T_k)]\bigr|_{t = \hat{t}} = \sigma_k^2 = \mu_k^2 (e^{s_k^2 \tau_k} - 1)
\label{eq:xVar}
\end{equation}
<!-- $s_k = \sqrt{Var[\Delta x_k / x_k(\hat{t})]}$ -->
<!-- Where the shorthand $\mu_k = E[x_k(T_k)]\bigr|_{t = \hat{t}}$ has been introduced. -->

Variance (or standard deviation) is a conventional measure of risk. For the formulation of the portfolio optimization problem farther below, it is expedient to introduce an alternative measure of risk, the coefficient of variation $c_k$.

\begin{equation}
c_k = \frac{\sigma_k}{\mu_k} = \sqrt{e^{s_k^2 \tau_k} - 1}
\label{eq:xCV}
\end{equation}
<!-- Where the shorthand $\sigma_k = \sqrt{Var[x_k(T_k)]\bigr|_{t = \hat{t}}}$ has been introduced. -->

The covariance between two project NPVs $x_k$ and $x_j$, meanwhile, is

\begin{equation}
\text{cov}[x_k(T_k), x_j(T_j)]\bigr|_{t = \hat{t}} = \mu_k \mu_j (e^{s_{kj} \sqrt{\tau_k \tau_j}} - 1) \:\:;\:\:\: s_{kj} = \text{cov}[\Delta x_k/x_k(\hat{t}), \Delta x_j/x_j(\hat{t})],\:\: s_{kk} = s_k^2
\label{eq:xCov}
\end{equation}

Where the shorthand $s_{kj}$ has been introduced to stand for the covariance between the quarterly arithmetic returns of projects $k$ and $j$. Now, note that this can be rearranged into an extension of the coefficient of variation as follows.

\begin{equation}
c_{kj} = \sqrt{\frac{\sigma_{kj}}{\mu_k \mu_j}} = \sqrt{e^{s_{kj} \sqrt{\tau_k \tau_j}} - 1} \:\:;\:\:\:\: \sigma_{kj} = \text{cov}[x_k(T_k), x_j(T_j)]\bigr|_{t = \hat{t}} \:\:\: \sigma_{kk} = \sigma_k^2 \:\:\: c_{kk} = c_k^2
\label{eq:xCcoV}
\end{equation}
<!-- Where $\rho_{kj} = \text{cor}(\Delta x_k / x_k(\hat{t}), \Delta x_j / x_j(\hat{t}))$. \rho_{i, j} s_k s_j \tau -->

This is the NPV coefficient of "covariation" between projects $k$ and $j$. It gives us the same type of information that the covariance or correlation does. Whereas the correlation coefficient is the covariance normalized by the product of the respective standard deviations, the coefficient of covariation is the covariance normalized by the product of the respective means.
<!-- Expected portfolio NPV $\mu(\hat{t})$  is then just the sum of the individual expected project NPVs. -->
<!-- \begin{equation} -->
<!-- \mu(\hat{t}) = \Sigma_k \mu_k(\hat{t}) -->
<!-- \end{equation} -->

## Measuring portfolio impact and risk

Expected portfolio impact (henceforth denoted $\mathcal{E}$) may be measured in a variety of ways. For present purposes, it is measured as the sum of squared project NPVs $\mu_k^2$ scaled by $r^2/2$.

\begin{equation}
\mathcal{E} = \frac{r^2}{2} \boldsymbol{\mu} ' \boldsymbol{\mu}
\label{eq:ErawPort}
\end{equation}

This is analogous to the kinetic energy of an $n$-body system in physics. (A technical explanation is given in the Appendix.)

Portfolio risk is measured as the portfolio standard deviation, defined

<!-- = Var[X(T)]\bigr|_{t = \hat{t}} -->
\begin{equation}
\begin{split}
\sigma(\hat{t}) &= \sqrt{\mathbf{1} ' \Sigma \mathbf{1}} \;\;;\;\;\; [\Sigma]_{kj} = \sigma_{kj}  \\
&= \sqrt{\boldsymbol{\mu}(\hat{t})' G \boldsymbol{\mu}(\hat{t})} \;\;;\;\;\; [G]_{kj} = c_{kj}
\end{split}
\label{eq:riskPort}
\end{equation}

Where $G$ is the project NPV "coefficient of covariation matrix". For computational purposes, it is helpful to note that $G$ may also be expressed
...
Where the notation $D(\boldsymbol{\mu})$ indicates a diagonal matrix whose diagonal elements are the elements of the vector $\boldsymbol{\mu}$; and $\Sigma_{\ell X}$ is the matrix of project log return covariances.
<!-- Where the elements of the vector $\boldsymbol{\tau}$ are the project time horizons $\tau_k$, and  -->
...
(Where the elements of the vector $\boldsymbol{\tau}$ are the respective project time horizons.) Having deduced $\tilde{\Sigma}_{\ell X}$, an approximate coefficient of covariation matrix $\tilde{G}$ then follows from Eq \ref{eq:G}.
...
Given the groundwork laid out above, the portfolio optimization problem may now be formulated and solved in terms of portfolio impact maximization subject to a risk tolerance threshold.
...
Where the portfolio risk $\sigma$ and expected impact $\mathcal{E}$ were defined in Eq \ref{eq:riskPort} and \ref{eq:ErawPort}.

As mentioned in the introduction, the control variables are defined as the project NPVs on the assumption that project NPV is a known function of the budget allocation. This is done in order to facilitate the mathematical exposition of the first and second order conditions. The optimal budget allocations are deduced from the optimal project NPVs at the end of this section.

Note that this formulation of the portfolio optimization problem lacks a budget constraint. It will soon become clear that no such constraint is necessary.

## First order conditions

The objective function or lagrangian corresponding to the optimization problem in Eq \ref{eq:probStat} is
...
Because eigenvectors are defined up to scaling, the constant $\beta$ may be arbitrarily assigned so as to enforce a portfolio impact, risk, or budget target/constraint. (The budget constraint is often the most binding of these three.) For this reason, no budget constraint was included in the objective function.
...
It then follows that only the candidate solution with the smallest eigenvalue results in a negative definite $H_j$ and is thus optimal, i.e., the maximizer of $\mathcal{L}$. Assuming eigenvalues are ordered decreasingly, the optimal solution is thus $\{\mathbf{q}_n, \upsilon_n\}$. By the same token, the candidate solution with the largest eigenvalue results in a positive definite $H_j$, and thus minimizes $\mathcal{L}$. This corresponds to the leading eigenvalue solution $\{\mathbf{q}_1, \upsilon_1\}$. The remaining in between solutions result in indefinite $H_j$, corresponding to saddle point solutions.
<!-- \begin{equation} -->
<!-- \begin{split} -->
...
The optimal _meaningful_ solution is thus $\{\mathbf{q}_{\hat n}, \upsilon_{\hat n}\}$.

## Deducing the optimal budget allocation from the optimal project NPVs \label{sec:dimRet}

Generally speaking, project NPV scales with investment. That is to say, beyond a certain minimum level of funding required to generate, release, and support the diffusion and uptake of the project's research product(s) on a pilot level, further increments in project investment have the effect of broadening the scope of impact---i.e. extending the size and number of populations or markets where the research product is released and has an impact. This scaling is not linear, but rather marginally diminishing. Project NPV $x_k(\hat{t})$ may thus be modeled as a marginally diminishing function of the funding allocation $w_k$, as follows.
...
Note that project NPV variances and covariances $\sigma_{jk}$ are also effectively functions of investment, since $\mu_k$ appears in their definition.
...
This expression can then be rearranged into an expression for the optimal budget allocation $w_k^*$.

\begin{equation}
w_k^* = w_k\bigr|_{\mu_k = \mu_k^*} C \left( \frac{\beta |q_k^*|}{\tilde{x}_k(0)} e^{-r T_k} \right)^{\frac{1}{\alpha_k}}
\label{eq:wFnmu}
\end{equation}

Note that $q_k^*$ can take negative values....complex 

## Dealing with negative project expected NPV

The eigenvectors $\mathbf{q}_j$ may include negative elements, which in turn implies negative expected project NPV. This is problematic because project NPVs are assumed to follow a gBm, which cannot take negative values. It is also problematic since a negative $q_k^*$ implies a complex valued budget allocation (Eq \ref{eq:wFnmu}). To redress this, here I introduce an alternative interpretation of negative NPV as the project value at the sunk cost point in the funding cycle. It is then possible to simply take the absolute value of $\abs{\mathbf{q}_j}$

<!-- As explained above, a negative expected NPV may be interpreted as the project value at the sunk cost point in the funding cycle; and negative values are problematic for portfolio optimization since it implies a complex valued budget allocation (Eq \ref{eq:wStarRaw}). The project time horizon $T_k$ is defined in Eq \ref{eq:Tk} so as to automatically extend or contract the time horizon by half a funding cycle, such that the budget allocations $\mathbf{w}_{kj}^*$ are guaranteed to be real valued. Note, moreover, that all of the terms in Eq \ref{eq:wStarRaw} are positive, whereby it is guaranteed that $\mathbf{w}_{kj}^*$ is also positive. -->
...
Finally, note that the impact to risk ratio may be expressed as follows.
...
[Where we justify the quadratic expression of portfolio impact...]
...
In the present context, $\mathcal{E}_k(t; T_k)$ is effectively a quadratic expression for the expected project level of effort expended over the course of the time horizon, expressed in units of value squared per unit time squared.
<!-- To discourage the present audience from getting too cozy with the physics analogy, $\mathcal{E}_k(t; T_k)$ is henceforth referred to using Gottfried Leibniz's more archaic, and now largely abandoned, term, "vis viva" (which means "living force" in Latin; for a reference, see [@iltis1971leibniz]). -->
...
Expected portfolio impact $\mathcal{E}$ is defined as the sum of project "kinetic energies".
...
[Where we justify taking the absolute value of expected impact...]
