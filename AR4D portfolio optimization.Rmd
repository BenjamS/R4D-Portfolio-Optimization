...
In this article, I propose a risk-adjusted portfolio optimization method to redress these issues. The proposed method solves for the risk minimizing allocation of funds, subject to a level of effort constraint, across a portfolio of research projects (or programs), each of which may have varying expected impacts, scaleability, time horizons, risk, synergies, and tradeoffs.
...
Where the portfolio risk $\sigma$ is defined $\sigma = \sqrt{\boldsymbol{\mu} ' G \boldsymbol{\mu}}$ (recall Eq \ref{eq:s2port}). Without loss of generality, the control variables $\mathbf{w}$ may be swapped for $\boldsymbol{\mu}$.
