...
\begin{equation}
\frac{\Delta \text{u}_k}{\Delta t} \Bigr|_{t = T} > \frac{\Delta \text{u}_k}{\Delta t} \Bigr|_{t = 0}
\end{equation}

This implies an acceleration $\Delta^2 \text{u}_k / \Delta t^2$ which comes as a result of the skilled, concerted, applied efforts that comprise the project intervention. Letting $F_k$ stand for these applied efforts and $a_k$ for the acceleration, this axiomatic relation can be expressed $F_k \sim a_k$, or
...
```{r, fig.show = "hold", fig.width = 4, fig.height=3, fig.align="center", fig.cap="\\label{fig:kinema}Generic example of an acceleration of a net benefit stream.", echo = FALSE}
...
\begin{equation}
\frac{\Delta \text{v}_k}{\Delta t} = \frac{\Delta^2 \text{u}_k}{\Delta t^2} = a_k
\end{equation}

Which can be rearranged as follows,

\begin{equation}
a_k \Delta t = \text{v}_k(T) - \text{v}_k(0)
\end{equation}
...
\begin{equation}
\begin{split}
\Delta \text{u}_k &= \text{v}_k(0) \Delta t + \int_0^T \text{v}_k dt \\
&= A_0 + \Delta A \\
&= \frac{1}{2} (\text{v}_k(T) + \text{v}_k(0)) \Delta t
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
a_k \Delta t \Delta \text{u}_k &= \frac{1}{2} (\text{v}_k(T) - \text{v}_k(0)) (\text{v}_k(T) + \text{v}_k(0)) \Delta t \\
a_k \Delta \text{u}_k &= \frac{1}{2} (\text{v}_k(T)^2 - \text{v}_k(0)^2)
\end{split}
\label{eq:kinema3}
\end{equation}
...
\begin{equation}
\mathcal{E}_k = \frac{\rho}{2} (\text{v}_k(T)^2 - \text{v}_k(0)^2)
\label{eq:Eraw}
\end{equation}
...
\begin{equation}
\mathcal{E}_k = \frac{\rho}{2} \text{v}_k(T)^2
\label{eq:Enpv}
\end{equation}
...
Portfolio velocity $\text{v}$ and vis viva $\mathcal{E}$ may be expressed

\begin{equation}
\begin{split}
\text{v} = \sqrt{\Sigma_k \left( \frac{\Delta \text{u}_k}{\Delta t} \right)^2} &= r \sqrt{ \Sigma_k \mu_k^2} \\
...
\begin{equation}
\mathcal{E} = \frac{\rho}{2} \text{v}^2 = \rho \frac{r^2}{2} \boldsymbol{\mu} ' \boldsymbol{\mu}
\label{eq:ErawPort}
\end{equation}
...
For ease of interpretation, the budget is normalized to 100, and the project maximum NPVs are expressed in the same units. An annual discount rate of `r rYrlyArith` is assumed, which works out to a quarterly discount rate of `r round(rQrtrlyArith, 3)`. The current maximum project NPV $\tilde{x}_k$ is usually reported in ex-ante impact assessments, or can be extrapolated from the information given in such assessments. Estimation of the scale elasticity parameter $\alpha_k$ is not currently a standard part of AR4D ex-ante impact assessments, but can be reasonably elicited through consultation with experts and literature. For commodity projects, given sufficient prior notice, both $\tilde{x}_k$ and $\alpha_k$ could, in principle, be estimated econometrically, using, for example, data gathered during field trials. Hypothetical values for $\tilde{x}_k$ and $\alpha_k$ are assigned to each project in the left panel of Fig \ref{fig:params}.
