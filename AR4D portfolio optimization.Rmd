...
Mills clearly identified and articulated the need for such a tool long ago [-@Mills1998], and yet work in this direction has not yet begun. Instead of portfolio optimization, AR4D institutions hold "priority setting" exercises, whereby research programs are ranked in order of importance based on their respective impact assessments. These rankings are then translated into research budget allocations in a highly subjective and ad hoc manner.

The priority setting process takes little or no account of the risk associated with individual research programs, much less of the synergies and tradeoffs (i.e. correlations) that may arise between programs. Research portfolios based on this approach are thus generally unbalanced and hence quite risky relative to a properly balanced portfolio. Conversely put, by neglecting research correlations, the priority setting method misses out on an opportunity to hedge research portfolio risk by balancing positive and negative correlations against each other.
...
A key challenge hindering work in this direction is the lack of data by which to calculate research correlations. In a previous article, I redress this problem by developing a method to deduce research correlation matrices based solely on expert opinion [-@schiek2021reverse]. Here I build upon that work to devise an AR4D portfolio optimization workflow.
<!-- solves for the risk-minimizing (i.e. optimally hedged) research budget allocation, subject to a portfolio impact constraint. -->

The proposed workflow is partly inspired by the portfolio optimization method originally devised in the financial context [markowitz, merton], but differs from that convention in several key respects. Firstly, in the financial context, expected returns scale linearly with investment. In the AR4D context, returns exhibit marginally diminishing returns to investment. Program NPV is defined to capture this reality.  Program NPV does not scale linearly with investment.  The budget constraint can be arbitrarily enforced 

The most radical departure... the financial problem is usually formulated as a risk minimization problem subject to a budget constraint and a return target constraint. Here I propose an alternative set up wherein the budget and return constraints are replaced by a single "energy" constraint, which I call the "vis viva".
...
In market or near market contexts where the principle of risk-neutral valuation may be invoked, $r$ is the risk-free rate of return. In the AR4D context... By definition, project NPV appreciates at the discount rate. Therefore the growth rates should be set to the discount rate $r$ ..., A project growth rate different from $r$ implies an assumption that the project $NPV$ is incorrectly calculated. 
<!-- If there are no good reasons for assigning a particular growth rate, then the $m_i$ should be set to a "natural" or fiat rate of appreciation, $r$. $r$ equal to the social discount rate, however the research institution and/or donor might define this. -->
...
\begin{equation}
\sigma^2 = Var[X(T)]\bigr|_{t = \hat{t}} = \boldsymbol{\mu}' K \boldsymbol{\mu} \end{equation}
...
## Deducing the value of variances and covariances
...
In the broadest terms, a project may be said to intervene in, or operate on, an existing net benefit stream $\Delta u/\Delta t$ with the aim of raising it to a higher value by the end of the project, such that
...
For the present purposes, it may be said that $F$ is constant over the life of the project, resulting in a similarly constant acceleration of the net benefit stream. This is illustrated in Figure \ref{fig:kinema}, where the shorthand $v = \Delta u/\Delta t$ has been introduced to reduce clutter.
...
gg <- gg + annotate("text", x = -0.06, y = yint,
                    label = expression(italic("v(0)")), size = label_size)
...
<!-- Now, consider that, for a given budget, the project aims to maximize the amount of skilled, concerted, applied effort ($F$) exerted by the project along the "displacement" $\Delta u$. That is to say, the aim is to maximize the quantity $F \Delta u$, subject to constraints (which will be addressed farther below).-->
...
\begin{equation}
\Delta u_i = \mu_i = x_i(\hat{t}) e^{r \tau}
\end{equation}

The corresponding project net-benefit stream and vis viva thus work out to

\begin{equation}
\frac{\Delta u_i}{\Delta t} = -r x_i(\hat{t}) e^{r \tau}
\end{equation}

[Comment on how to interpret negative velocity -- backwards looking?]

and

\begin{equation}
\begin{split}
\mathcal{E}_i &= \frac{\lambda}{2} r^2 x_i(\hat{t})^2 e^{2r \tau} \\
&= \frac{\lambda}{2} r e^{r \tau} \mu_i^2
\end{split}
\end{equation}

Respectively. And the net-benefit stream and vis viva of an entire portfolio are

\begin{equation}
\frac{\Delta u}{\Delta t} = -r e^{r \tau} \mathbf{x}(\hat{t}) ' \mathbf{1}
\end{equation}
...
\begin{equation}
\begin{split}
\mathcal{E} &= \frac{\lambda}{2} r^2 e^{2r \tau} \mathbf{x}(\hat{t}) ' \mathbf{x}(\hat{t}) \\
&= \frac{\lambda}{2} r e^{r \tau} \boldsymbol{\mu} ' \boldsymbol{\mu}
\end{split}
\end{equation}
...
# Risk-adjusted AR4D portfolio optimization replacing cost and return constraints with a vis viva constraint

The risk adjusted portfolio optimization problem can now be formulated replacing the budget constraint with a vis viva target. [No need for the budget constraint as this is trivially enforced through the parameter $C$.]

[$n$ portfolio items]

[Comment on nabla_mu vs. nabla_w]

\begin{equation}
\min_{\mathbf{w}}{\sigma} \:\:\: s.t. \:\:\: \mathcal{E} = \overline{\mathcal{E}}
\end{equation}
...
\begin{equation}
\begin{split}
\mathcal{L} &= \sigma + \lambda (\mathcal{E} - \overline{\mathcal{E}}) \\
&= \sqrt{\boldsymbol{\mu} ' K \boldsymbol{\mu}} - \frac{\lambda}{2} r e^{r \tau} \boldsymbol{\mu} ' \boldsymbol{\mu} - \overline{\mathcal{E}})
\end{split}
\end{equation}
...
\begin{equation}
\nabla_{\mu} \mathcal{L} = \frac{e^{r \tau}}{\sigma} K \boldsymbol{\mu} + \lambda r^2 e^{r \tau} \boldsymbol{\mu} = \mathbf{0}
\end{equation}

The optimal project NPV vector $\boldsymbol{\mu}^*$ is thus given by any of the eigenvectors of $K$, with corresponding vis viva shadow price $\lambda$ given by the respective eigenvalues.

[The project vis viva is proportional to the risk gradient.]

The optimal budget allocation is then

\begin{equation}
\mathbf{w}^* = C (D(\mathbf{k})^{-1} \boldsymbol{\mu}^* )^{\frac{1}{\boldsymbol{\alpha}}}
\label{eq:wStar}
\end{equation}

## The vis viva shadow price

Note that $\lambda$ is the marginal benefit of a marginal increment in vis viva above the target $\overline{\mathcal{E}}$. That is,
...
The second order condition guaranteeing that the solution $\boldsymbol{\mu^*}$ is a minimizer of $\mathcal{L}$ is

\begin{equation}
\mathbf{q} ' H \mathbf{q} > 0 \:|\: \mathbf{q} ' \nabla \mathcal{E} = 0
\end{equation}
...
And $q$ is any nonzero, real valued vector in the null space of the constraint gradient $\nabla \mathcal{E}$.

To resolve this condition, note that $\mathbf{q}$ can be rewritten in terms of an arbitrary, nonzero, real valued vector $\mathbf{y}$ multiplied by a basis $Z$ whose columns are in the null space of $\nabla \mathcal{E}$.

\begin{equation}
Z \mathbf{y} = \mathbf{q} \:|\: \nabla \mathcal{E} ' Z = \mathbf{0}
\end{equation}

Since $\nabla_{\boldsymbol{\mu}} \mathcal{E}$ is any eigenvector of $K$, say the $k^{th}$ eigenvector, then the other eigenvectors constitute a valid basis $Z$. Letting the notation $P_k$ stand for the eigenvector matrix $P$ excluding the $k^{th}$ column, then $Z = P_k$. The second order condition for a minimum can thus be rewritten

\begin{equation}
\mathbf{y} ' H_k \mathbf{y} &> 0
\end{equation}

Where $H_k$ is the projected Hessian of $\mathcal{L}$ evaluated at the $k^{th}$ solution $\boldsymbol{\mu}^* = \mathbf{p}_k$.

\begin{equation}
H_k = P_k ' \nabla_{\boldsymbol{\mu}}^2 \mathcal{L}|_{\boldsymbol{\mu}^* = \mathbf{p}_k} P_k
\end{equation}

The second order condition is thus fulfilled if and only if $H_k$ is positive definite. This is easy to determine since $H_k$ is a diagonal matrix.

\begin{equation}
\begin{split}
H_k &= P_k ' K P_k - \gamma_k P_k ' I P_k \\
&= \Gamma_k - \gamma_k I
\end{split}
\end{equation}

Where $\Gamma_k$ is the diagonal matrix of eigenvalues of $K$ excluding the $k^{th}$ eigenvalue ($\gamma_k$). Since $K$ is positive definite, it is straightforward to see that $H_n$ is positive definite (i.e., the case where \boldsymbol{\mu}^* equals the trailing eigenvector of $K$). By the same token, $H_1$ is negative definite (i.e., the case where $\boldsymbol{\mu}^*$ equals the leading eigenvector of $K$), and the matrices $H_{1 < k < n}$ are indefinite.
...
In relation to the portfolio optimization method presented above, this implies that each of the $m$ portfolio solutions $\boldsymbol{\mu}^*$ is tied to a specific strategic objective. Selection of any one of these solutions thus implies an exclusive focus on a particular strategic objective, and neglect of the rest. This is problematic since AR4D institutions and donors are generally interested in optimizing budget allocations across multiple strategic objectives at once. They want to pursue, for example, nutritional security, economic growth, and smallholder resilience, all at once, and not one at a time, in a piecemeal fashion. Indeed, such objectives usually go hand in hand, and cannot be pursued in isolation from each other.
...
cov_shrink(as.matrix(cormat_XX_derived), )
...
xx <- Ptilde %*% portWgts
xx <- xx - 1.1 * min(xx)
scaleFctrs <- diag(1 / portWgts) %*% t(Ptilde) %*% xx
Ptilde %*% diag(portWgts) %*% scaleFctrs
xx <- Ptilde %*% diag(portWgts) %*% scaleFctrs
k <- runif(nrow(Ptilde)) * 10
a <- runif(nrow(Ptilde))
r <- 0.001
tau <- 40
sol <- (xx / k)^(1 / a) * exp(-r * tau)
C <- sum(sol)
sol <- sol / C


Ptilde %*% diag(eVecp[, 2])
eMatProj %*% eMatPolicy[, 3]
eMatProj %*% eMatPolicy[, 4]


#eigen(covmatSSderived)$vectors
#=============================================================
# Experiment
nc <- 3
nr <- 4
...
xx2 <- xx1 - 1.1 * min(xx1)
scaleFctrs <- xx2 / xx1
diag(c(scaleFctrs)) %*% Ptilde %*% wgts

y <- diag(1 / wgts) %*% t(Ptilde) %*% xx2
...
# slackfn <- function(scaleFctrs){
#   
#   slack <- Ptilde %*% diag(c(scaleFctrs)) %*% wgts - sol
#   
# }
# nleqslv::nleqslv()

Ptilde %*% diag(c(scaleFctrs)) %*% scaleFctrs

#xx <- xx / sum(xx)
#Q <- Ptilde %*% diag(wgts)
scaleFctrs <- diag(1 / wgts) %*% t(Ptilde) %*% xx
#scaleFctrs <- diag(1 / wgts^2) %*% t(Q) %*% xx
xx <- Ptilde %*% diag(wgts) %*% scaleFctrs

diag(1 / wgts^2) %*% t(Ptilde) %*% xx
...
Q <- Ptilde %*% eVecp
Q
Q <- Ptilde %*% eVecp %*% rep(1, nc)
Q

#Q <- Ptilde %*% diag(eVecp[, 3])
...
Qd <- Q[irow, ] - xmin
Qd[icol] <- 10^6
iColAdj <- which(Qd == min(Qd))
xadj <- Q[irow, iColAdj]
fctr <- -xadj / xmin
Q[, iColAdj] + Q[, icol] * fctr

Q %*% rep(1, nc)


Qd <- (Q) - xmin
indAdj <- which(Qd == min(Qd))
indCol <- ceiling(indAdj / nr)
xadj <- Q[indAdj]
fctr <- xadj / xmin
#Q[, indCol] <- Q[, indCol] * fctr
Q <- Ptilde %*% diag(eVecp[, 1])
Q[, indCol] * fctr + Q[, 2]
...
wgts <- 1 / eValp #rep(min(Ptilde), nc)
Ptilde %*% diag(eVecp[, 1]) %*% wgts
