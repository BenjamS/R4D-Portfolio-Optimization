...
By definition, program NPV appreciates over time at the monthly discount rate $r$. Therefore, the growth rate $m$ should be set equal to $r$. A program growth rate different from $r$ implies that the program $NPV$ is inaccurately calculated.
...
\begin{equation}
E[x_k(T)]\bigr|_{t = \hat{t}} = x_k(\hat{t}) e^{r \tau}
\label{eq:ExTraw}
\end{equation}
...
\begin{equation}
\sigma(\hat{t})^2 = \boldsymbol{\mu}' G_{X,X} \boldsymbol{\mu} \end{equation}

Where $G_{X,X}$ is the "cross coefficient of variation matrix", i.e. the matrix whose elements are $c_{kj}$, where $c_{kk} = c_k^2$. Note that $G$ is just the covariance matrix left and right multiplied by a diagonal matrix of the inverse project means.
...
Alternatively, $G$ can be expressed in terms of the correlation matrix ($K$).

\begin{equation}
G = D(\mathbf{c}) K D(\mathbf{c})
\end{equation}

It follows that $G$ has the same eigenvalues as $K$ and $\Sigma$; and that the eigenvectors of $G$ are just the eigenvectors of $K$ premultiplied by $D(\mathbf{c})$. That is to say, if the eigendecompositions of $G$ and $K$ are denoted, respectively,
...
Then the eigenvectors of $G$ and $K$ are related as follows.
...
\begin{equation}
\frac{\partial \ln(x_k)}{\partial \ln(w_k)} = \alpha_k
\end{equation}

And $\alpha_k$ lies between $0$ and $1$, reflecting marginally diminishing returns to investment.

[not the only marginally diminishing fn form, may also want to experiment with $\tilde{x}_k e^{-C/w_k}$---which includes an area of marginally increasing returns before they turn decreasing]

Substituting this into equation \ref{eq:ExTraw}, program NPV can now be rewritten as a function of funding level.

\begin{equation}
\mu_k(\hat{t}) = \tilde{x}_k(\hat{t}) \left( \frac{w_k}{C} \right)^{\alpha_k} e^{r T}
\label{eq:muFnw}
\end{equation}
<!-- ... -->
...
## Deducing program NPV variances and covariances \label{sec:volEst}
...
Program NPV ($\tilde{x}_k(\hat{t})$) can be estimated at the start of the funding cycle based on ex-ante impact assessment. But program NPV variance ($\sigma_k^2$) is less straightforward, and covariances between programs ($\sigma_{kj}$) even less so.

Elsewhere I develop a crowdsourcing method to estimate the AR4D correlation matrix based on principal components analysis [-@]. To convert these correlations into covariances, the program variances are required. More specifically, the standard deviation of percentage changes in NPV, $s_k$, is required.

While it may be difficult to develop an intuitive grasp of the value of $s_k$, research managers (and/or stakeholders and/or the foresight economists conducting the program appraisal) generally have some idea of the uncertainty surrounding NPV estimates, such that they can be asked to quantify minimum and maximum bounds on the NPV. For example, they can say that NPV is $\tilde{x}(\hat{t})$, but that it might be six times that amount, or half that amount. (Or they may give the same information in terms of magnitude, which is then easily transformed into percentage terms.)
...
\begin{equation}
\begin{split}
E[\ell] \bigr|_{t = \hat{t}} &= \left(r - \frac{s_k^2}{2} \right) \tau \\
Var[\ell] \bigr|_{t = \hat{t}} &= s_k^2 \tau
\end{split}
\end{equation}

The upper and lower bounds on the log return, $\bar{\ell}$ and $\underline{\ell}$, may thus be expressed as follows.

\begin{equation}
\begin{split}
\bar{\ell} &= E[\ell] \bigr|_{t = \hat{t}} + z \sqrt{Var[\ell] \bigr|_{t = \hat{t}}} \\
\underline{\ell} &= E[\ell] \bigr|_{t = \hat{t}} - z \sqrt{Var[\ell] \bigr|_{t = \hat{t}}}
\end{split}
\end{equation}

Where $z = 1.96$ is the standard score corresponding to the $95\%$ confidence interval of a normally distributed random variable. The standard score $z$ can of course be adjusted to match the practitioner's idea of a suitable confidence interval defined by the bounds $\bar{\ell}$ and $\underline{\ell}$.

This system of equations can then be rearranged into three expressions for the volatility parameter $s_k$ in terms of known parameters (i.e. in terms of the upper and lower log returns $\bar{\ell}, \underline{\ell}$, the discount rate $r$, the project time horizon $T$, and the standard score $z$).

\begin{equation}
\begin{split}
s_k &= \frac{z \pm \sqrt{z^2 - 1 / 2 (\bar{\ell} - r \tau)}}{ \sqrt{\tau}} \\
&= \frac{-z \pm \sqrt{z^2 - 1 / 2 (\underline{\ell} - r \tau)}}{ \sqrt{\tau}} \\
&= \sqrt{ r - \frac{\bar{\ell} + \underline{\ell}}{2 \tau}}
\end{split}
\end{equation}

Once $s_k$ has been deduced in this way for each program, it is then straightforward to compute the variances $\boldsymbol{\sigma}$ and coefficients of variation $\mathbf{c}$. With these in hand,[together with the crowdsourced correlation matrix $\tilde{K}$,] the deduced covariance matrix and cross coefficient of variation matrix follow as

\begin{equation}
\tilde{\Sigma} = D(\boldsymbol{\sigma}) \tilde{K} D(\boldsymbol{\sigma})
\end{equation}

And

\begin{equation}
\tilde{G} = D(\mathbf{c}) \tilde{K} D(\mathbf{c})
\end{equation}
...
# Program NPV is cyclically real \label{sec:cycReal}
...
\begin{equation}
\mu_k(0) = x_k(0) e^{(\omega + i) 2 \pi} \;\;;\;\;\; \omega = r \frac{T}{2 \pi}
\end{equation}

Where $i = \sqrt{-1}$. (Recall that $e^{2\kappa i \pi} = 1$ for any integer $\kappa$.) This may then be extended to all angles $\theta \in (0, 2 \pi)$ as follows

\begin{equation}
\mu_k(\theta; T, r) = x_k(\theta) e^{(2 \pi - \theta) (\omega + i)}
\end{equation}

In this formulation, program value $\mu_k(\theta; T, r)$ is real-valued at the start of the funding cycle ($\theta = 0$), midway through ($\theta = \pi$), and at the end of the cycle ($\theta = 2 \pi$); and is complex-valued elsewhere. Real program values thus correspond to the points in the funding cycle when the investment has a tangible or demonstrable form---particularly at the beginning, when the funds are disbursed, and at the end, when the investment has been productively consumed and transformed into measurable program impacts. The negative real value occurring halfway through the cycle, at $\theta = \pi$, may be interpreted as the investment as a sunk cost. Complex values lying off the real number line, meanwhile, correspond to points in the funding cycle when the investment has no demonstrable or tangible form, i.e. when it is in the process of being transformed from its original monetary form into the planned program impacts.
...
Extending the time horizon by one half cycle ($\pi$) thus results in the appearance of $-\mu_k(0)$, which is positive, thereby making the budget allocation $w_k$ real valued.

Note that the same result is achieved by using any odd integer multiple of $\pi$. That is, $w_k(\mu_k(0), \tilde{x}_k(0), \omega, (2 \kappa - 1) \pi)$ ...

\begin{equation}
w_k = C \left( \frac{\mu_k(0)}{\tilde{x}_k(0)} e^{-(2 \kappa - 1) \pi (\omega + i)} \right)^{\frac{1}{\alpha_k}}
\end{equation}


It is therefore useful to introduce an integer parameter $\kappa_k$ into the definition of each program NPV




For the purposes of portfolio optimization, it is useful to introduce an integer parameter by which each program funding time horizon can be adjusted in half cycle intervals with respect to the funding cycle $T$.

\begin{equation}
\begin{split}
\mu_k(0; r, T) &= x_k(0) (-1)^{2 \kappa_k - 1} e^{2 \pi \omega \left(\kappa_k + \frac{1}{2} \right))} \\
&= x_k(0) e^{2 \pi \omega \left(\kappa_k + \frac{1}{2} \right) + 2 i \pi \left(\kappa_k + \frac{1}{2} \right)}
\end{split}
\end{equation}
...
Note that if $\mu_k(\theta; T, r)$ is a positive real number at the start of the funding cycle ($\theta = 0$), then it must also be a positive real number at the end of the funding cycle ($\theta = 2 \pi$). Likewise, if program value is a negative real number at the start of a funding cycle, then it must also be negative at the end of the funding cycle. Funding cycles that begin and end on a positive real value may be called "normal funding cycles". Cycles that begin and end on a negative value may thus be said to be out of phase with the normal funding cycle by a rotation of $\pi$---i.e. by half a cycle.

It follows that negative program values at the start of the cycle ($\theta = 0$) may be turned positive by extending or shortening the time horizon by $\pi$. That is, starting with an out of phase program value at $\theta = 0$,

\begin{equation}
\begin{split}
\mu_k(0; r, T) &= -x_k(0) e^{(\omega + i) 2 \pi} \\
&= x_k(0) e^{(\omega + i) 2 \pi + i \pi}
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\mu_k(0; r, 3 / 2 T) &= -x_k(0) e^{(\omega + i) 3 \pi} \\
&= x_k(0) e^{(\omega + i) 3 \pi + i \pi} \\
&= x_k(0) e^{3 \omega \pi + 4 i \pi} \\
&= x_k(0) e^{3 \omega \pi}
\end{split}
\end{equation}

[Extending the time horizon in this way has the effect of a clockwise $\pi$ rotation of $\mu_k(0; r , T)$.] Alternatively, the program value may be made positive by shortening the time horizon by $T / 2$, [which corresponds to a counter-clockwise $\pi$ rotation in the complex plane.]

\begin{equation}
\begin{split}
\mu_k(0; r, T / 2 ) &= -x_k(0) e^{(\omega + i) \pi} \\
&= x_k(0) e^{(\omega + i) \pi + i \pi} \\
&= x_k(0) e^{ \omega \pi + 2 i \pi} \\
&= x_k(0) e^{ \omega \pi}
\end{split}
\end{equation}

More generally, a negative program value can be brought into phase by extending or shortening the time horizon by $(2\kappa - 1) T / 2$, where $\kappa$ is a positive integer. [In some decision making situations, it might make sense to consider n < 0, which extends the time horizon backwards---corresponding perhaps to funding of a process already in mid course.]
...
The portfolio optimization problem may now be formulated in terms of vis viva maximization subject to a risk tolerance threshold.
...
The solutions given by Eq \ref{eq:NPVstar}, in turn, can be solved for their corresponding budget allocations $\mathbf{w}_j^*$ via Eq \ref{eq:muFnw}, as follows.
<!-- The candidate budget allocations associated with the solutions in  can be found by solving equation \ref{eq:muFnw} for $\mathbf{w}$ and evaluating at $\boldsymbol{\mu}_j^*$, as follows. -->
...
\begin{equation}
\begin{split}
\mathbf{w}_j^* &= C (D(\tilde{\mathbf{x}}(0))^{-1} \mathbf{q}_j e^{-r T})^{\frac{1}{\boldsymbol{\alpha}}}
\label{eq:wStar}
\end{equation}
<!-- The individual program budget allocations are thus given by -->
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \mathbf{w}_{kj}^* &= C \left(\frac{q_{kj}}{\tilde{x}(0)} e^{-r T} \right)^{\frac{1}{\alpha_k} -->
<!-- \end{equation} -->
...
### Dealing with negative elements in the solution

Generally speaking, the eigenvectors $\mathbf{q}_j$ may include negative elements, which in turn implies negative expected program NPV (equation \ref{eq:NPVstar}) and complex valued budget allocations (equation \ref{eq:wStar}). As explained in \ref{sec:cycReal}, negative program NPV may be interpreted as the NPV evaluated at the sunk cost point of the program funding cycle. Insofar as this interpretation is valid, negative NPV can be made positive by extending or shortening the program's time horizon by an odd integer multiple of one half the original time horizon $T$. Mathematically, this means multiplying the negative element $q_{kj} < 0$ by $-e^{rT (\kappa + 1/2)}$, where $\kappa$ is an integer. The "rotated" or "horizon adjusted" optimal program budget allocation $\hat{w}_{kj}^*$ and NPV $\hat{\mu}_{kj}^*$ are thus defined

\begin{equation}
\hat{w}_{kj}^* =
\begin{cases}
C \left( \frac{\beta_j q_{kj}}{\tilde{x}(0)} e^{-r T} \right)^{\frac{1}{\alpha_k}}, & \text{if $q_{kj} > 0$} \\
C \left( - \frac{\beta_j q_{kj}}{\tilde{x}(0)} e^{-r T \left(\frac{1}{2} + \kappa \right)} \right)^{\frac{1}{\alpha_k}}, & \text{if $q_{kj} < 0$}
\end{cases}
\end{equation}
...
\hat{\mu}_{kj}^* =
\begin{cases}
q_{kj}, & \text{if $q_{kj} > 0$} \\
-e^{rT \left(\kappa - \frac{1}{2}\right)} q_{kj}, & \text{if $q_{kj} < 0$}
\end{cases}
\end{equation}
<!-- The choice of $\kappa$ should be made depending on whether it makes more sense to shorten or to lengthen the time horizon of the program in question, and by how much. -->

Or, more compactly,

\begin{equation}
\hat{w}_{kj}^* = \Sigma_{j = 1}^m C ( \frac{\beta_j q_{kj}}{\tilde{x}(0)} (-1)^{2 \kappa - 1} e^{-r T \left(\frac{1}{2} + \kappa \right)})^{\frac{1}{\alpha_k}}
\end{equation}

And

\begin{equation}
\hat{\mu}_{kj}^* = (-1)^{1 - 2 \kappa} e^{rT \left(\kappa - \frac{1}{2}\right)} \beta_j q_{kj}
\end{equation}
...
\kappa
\begin{cases}
= \frac{1}{2}, & \text{if $q_{kj} > 0$} \\
\in \mathbb{Z} & \text{if $q_{kj} < 0$}
\end{cases}
\end{equation}
...
Where $\beta_j$ is an arbitrary scaling constant, which may be chosen such that $\hat{\mathbf{w}}_j^* {'} \mathbf{1} = C$.
...
<!-- \begin{equation} -->
<!-- \boldsymbol{\mu}_j^* = (-1)^{1 - 2 \kappa} e^{rT \left(\kappa - \frac{1}{2}\right)} \beta_j \mathbf{q}_j -->
<!-- \end{equation} -->
\begin{equation}
\begin{split}
\hat{\mathcal{E}}_j^* &= \frac{1}{2} \hat{\mathbf{v}}_j^* ' \hat{\mathbf{v}}_j^* = \frac{r^2}{2} \hat{\boldsymbol{\mu}}_j^* ' \hat{\boldsymbol{\mu}}_j^* \\
&= \frac{r^2}{2} \mathbf{q}_j ' \mathbf{q}_j = \frac{r^2}{2} \beta_j^2 \\
\sigma_j^* &= \sqrt{\boldsymbol{\mu}_j^* ' G_{X, X} \boldsymbol{\mu}_j^*} = \sqrt{\mathbf{q}_j ' G_{X, X} \mathbf{q}_j} \\
&= \beta_j \gamma_j
\end{split}
\end{equation}

More explicitly, the individual candidate program budget shares can be expressed

\begin{equation}
\begin{split}
w_{kj}^* &= C \left( \frac{\beta_j q_{kj}}{\tilde{x}(0)} e^{-r T} \right)^{\frac{1}{\alpha_k}} \\
&= C \left( \frac{\beta_j c_k p_{kj}}{\tilde{x}(0)} e^{-r T} \right)^{\frac{1}{\alpha_k}}
\end{split}
\end{equation}

## A menu of solutions catering to a range of strategic objectives

Generally speaking, each of the $m$ leading eigenvectors of a covariance matrix of a complex system of variables encode information about a specific aspect of the system. In the financial context, for example, Gopikrishnan et al. [-@gopik2001quant] show that the each of the leading eigenvectors of a covariance matrix of hundreds of stock price series is associated with movements in a particular industry or sector, such that each element in the eigenvectors reflects a particular stock's contribution to, or participation in, movements in the respective sector.
<!-- [The authors even suggest this method might be used to classify firms into industries.] -->

Elsewhere, I extend the logic of Gopikrishnan et al.'s analysis to AR4D correlation matrices [-@schiek2021reverse]. In that extension, I show that the collective movements associated with each eigenvector may be interpreted as the AR4D institution's (and/or donor's) strategic objectives or policies, such that the elements of the eigenvectors may be interpreted as reflecting each research program's contribution towards the respective policy.
...
<!-- In my previous work, I show how this matrix, which plays a central role in the next section, can be "crowdsourced"... -->
As discussed in section ... , This is a very useful matrix, which may reasonably be deduced from a survey of experts and stakeholders. The outer product $K_{X, S}^{\circlearrowright} K_{X, S}^{\circlearrowright}{'}$ gives the approximate program correlation matrix $\tilde{K}_{X, X}$, while the inner product $K_{X, S}^{\circlearrowright} {'} K_{X, S}^{\circlearrowright}$ gives the policy covariance matrix (call this $\Sigma_{S, S}^{\circlearrowright}$). In the absence of time series data by which to estimate program and policy correlations, then, the matrix $K_{X, S}^{\circlearrowright} = B \tilde{P}$ provides a means of deducing them from expert opinion. (See Schiek [-@schiek2021reverse] for details.)
...
Alternatively, the investor may want to optimize across multiple strategic objectives. In other words, they may want a budget allocation $\mathbf{w}^{**}$ that is some optimal linear combination of the budget allocations on the optimal menu.

\begin{equation}
\begin{split}
\mathbf{w}^{**} &= W^* \boldsymbol{\nu}^* \\
&= \Sigma_{j = 1}^m \mathbf{w}_j^* \nu_j^*
\end{split}
\end{equation}

The optimal policy weight vector $\boldsymbol{\nu}^*$ can be determined by solving the portfolio problem stated in equation \ref{eq:probStat} at the policy level.

\begin{equation}
\max_{\boldsymbol{\mu}^S} \mathcal{E}^S \:\:\:\:s.t.\:\:\: \sigma^S = \bar{\sigma}^S
\end{equation}

Where $\mathcal{E}^S$ and $\sigma^S$ are the policy vis viva and risk, defined as the policy level aggregation of the program level solutions.

\begin{equation}
\begin{split}
\mathcal{E}^S &= \Sigma_{j = 1}^m \mathcal{E}_j^* = \frac{1}{2} \Sigma_j \mathbf{v}_j^* ' \mathbf{v}_j^* \\
&= \frac{r^2}{2} \Sigma_j \boldsymbol{\mu}_j^* ' \boldsymbol{\mu}_j^*
\sigma^S &= \mathbf{1} ' \Sigma_{S, S} \mathbf{1} = \boldsymbol{\mu}^S ' G_{S,S} \boldsymbol{\mu}^S
\end{split}
\end{equation}

Where $G_{S,S}$ is the policy cross coefficient of variation matrix, the control variables $\boldsymbol{\mu}^S$ are the expected policy NPVs, and the elements of $\boldsymbol{\mu}^S$ are defined

\begin{equation}
\mu_j^S = \boldsymbol{\mu}_j^* ' \mathbf{1}
\end{equation}
...
\begin{equation}
\boldsymbol{\mu}^S = \boldsymbol{\beta}
\end{equation}

And so the policy optimization problem can be rewritten

\begin{equation}
\max_{\boldsymbol{\beta}} \mathcal{E}^S \:\:\:\:s.t.\:\:\: \sigma^S = \bar{\sigma}^S
\end{equation}
...
\begin{equation}
\begin{split}
\mathcal{E}^S &= \frac{r^2}{2} \boldsymbol{\beta} ' \boldsymbol{\beta} \\
\sigma^S &= \boldsymbol{\beta} ' G_{S,S} \boldsymbol{\beta}
\end{split}
\end{equation}
<!-- That is, the policy problem is one of finding the eigenvector scaling constants $\boldsymbol{\beta}^*$ that maximize policy vis viva (impact) subject to a risk tolerance threshold. -->

The corresponding lagrangian is then

\begin{equation}
\begin{split}
\mathcal{L} &= \mathcal{E}^S - \lambda^S (\sigma^S - \bar{\sigma}^S) \\
&= \frac{r^2}{2} \boldsymbol{\beta} ' \boldsymbol{\beta} - \lambda^S (\boldsymbol{\beta} ' G_{S,S} \boldsymbol{\beta} - \bar{\sigma}^S)
\end{split}
\end{equation}
...
\begin{equation}
\nabla_{\boldsymbol{\beta}} \mathcal{L} = r^2 \boldsymbol{\beta} - \frac{\lambda^S}{\sigma^S} G_{S, S} \boldsymbol{\beta} = \mathbf{0}
\label{eq:focS}
\end{equation}
...
The candidate solution $\boldsymbol{\beta}^*$ may thus be any of the eigenvectors of $G_{S, S}$, with corresponding policy risk shadow price ($\lambda^S$) given in terms of the respective eigenvalue. The eigenvectors and eigenvalues of $G_{S, S}$ may be deduced from the crowdsourced matrix $K_{X, S}^{\circlearrowright}$ as follows.

First, recall from section ... that the policy covariance matrix $\Sigma_{S, S}$ can be obtained as the inner product $K_{X, S}^{\circlearrowright} {'} K_{X, S}^{\circlearrowright}$. Moreover, note that the eigendecomposition of $\Sigma_{S, S}^{\circlearrowright}$ works out to

\begin{equation}
\Sigma_{S, S}^{\circlearrowright} = B \tilde{\Gamma} B '
\end{equation}

That is to say, the matrix of eigenvectors of $\Sigma_{S, S}^{\circlearrowright}$ is just the orthonormal rotation matrix $B$ that is applied to clarify program-policy correlations (recall equation \ref{eq:rotL}); and the respective eigenvalues are just the $m$ leading eigenvalues of the program cross coefficient of variation matrix $G_{X, X}$ (or program correlation matrix $K_{X, X}$). (See Schiek [-@schiek2021reverse] for details.)

Since $G_{S, S} = D(\boldsymbol{\beta})^{-1} \Sigma_{S, S} D(\boldsymbol{\beta})^{-1}$ by definition, it follows that the eigenvalues of $G_{S, S}$ are likewise $\tilde{\Gamma}$. Combining this information with the first order conditions (equation \ref{eq:focS}), it follows that $\sigma_j^{S*} = \sqrt{\gamma_j} = \sigma_j^*$, and

\begin{equation}
\begin{split}
\lambda^S_j &= r^2 \frac{\sigma_j^{S*}}{\gamma_j} \\
&= \frac{r^2}{\sqrt{\gamma_j}}
&= \lambda_j
\end{split}
\end{equation}

That is, the optimal policy portfolio risk and risk shadow price are equal to the optimal program portfolio risk and risk shadow price.

Moreover, the eigenvectors of $G_{S, S}$ and \Sigma_{S, S}^{\circlearrowright} are trivially related as follows. Letting $\boldsymbol{\varepsilon}_j$ and $\mathbf{b}_j$ stand for the $j^{th}$ eigenvectors of $G_{S, S}$ and $\Sigma_{S, S}^{\circlearrowright}$, respectively,
...
To avoid complex valued elements in $\boldsymbol{\beta}_j$, recall that negative elements in $\mathbf{b}_j$ can be made positive by adjusting the program time horizon, as explained in section \ref{sec:cycReal}.

Deduction of the second order condition for a maximum follows the same procedure as in the program level problem (section \ref{sec:soc}), and results in an analogous conclusion: the investor is presented not with a single optimal linear combination, but with an optimal menu of linear combinations, each corresponding to a particular portfolio risk tolerance (\sqrt{\gamma_{\ell}}). [The investor should choose the solution $\boldsymbol{\beta}_{\ell}^*$ with the risk shadow price $\lambda_{\ell}$ that best matches their risk tolerance.]

With the optimal policy weights ($\boldsymbol{\beta}_{\ell}^*$) in hand, the optimal multi-objective program NPV vector $\boldsymbol{\mu}_{\ell}^*$ follows as the optimal linear combination of the optimal program NPV menu.

\begin{equation}
\begin{split}
\boldsymbol{\mu}_{\ell}^* &= \Sigma_{j = 1}^m \mu_j^* \beta_{j \ell}^* \\
\boldsymbol{\mu}_{\ell}^* &= Q \boldsymbol{\beta}_{\ell}^* \\
&= D(\mathbf{c}) P \boldsymbol{\beta}_{\ell}^*
\end{split}
\end{equation}

The optimal multi-objective impact $\mathcal{E}_{\ell}^{S*}$ then follows as

\begin{equation}
\begin{split}
\mathcal{E}_{\ell}^{S*} &= \frac{r^2}{2} \boldsymbol{\mu}_{\ell}^* ' \boldsymbol{\mu}_{\ell}^* \\
&= \frac{r^2}{2} \boldsymbol{\beta}_{\ell}^* Q ' Q \boldsymbol{\beta}_{\ell}^* \\
&= \frac{r^2}{2} \boldsymbol{\beta}_{\ell}^* ' \boldsymbol{\beta}_{\ell}^*
\end{split}
\end{equation}
...
\begin{equation}
\mathbf{w}_{\ell}^* = \Sigma_{j = 1}^m C_j (D(\tilde{\mathbf{x}}(0))^{-1} \beta_{j \ell}^* \mathbf{q}_j (-1)^{2 \kappa - 1} e^{-r T \left(\frac{1}{2} + \kappa \right)})^{\frac{1}{\boldsymbol{\alpha}}}
\label{eq:wStarPolicy}
\end{equation}
...
\begin{equation}
\begin{split}
\mathbf{w}_{\ell}^* &= \boldsymbol{\mu}_{\ell}^* ' \boldsymbol{\mu}_{\ell}^* \\
&= \frac{r^2}{2} \boldsymbol{\beta}_{\ell}^* Q ' Q \boldsymbol{\beta}_{\ell}^* \\
&= \frac{r^2}{2} \boldsymbol{\beta}_{\ell}^* ' \boldsymbol{\beta}_{\ell}^*
\end{split}
\end{equation}
...
To enforce the budget constraint...scale to enforce budget constraint but know this scales risk too



<!-- \begin{equation} -->
<!-- G_{S, S} = D() K_{X, S}^{\circlearrowright} ' K_{X, S}^{\circlearrowright} -->
<!-- \end{equation} -->



<!-- ...an approximate program NPV correlation matrix $\tilde{K}$ as the outer product of this matrix with itself. -->
<!-- \begin{equation} -->
<!-- \tilde{K} = K_{X, S}^{\circlearrowright} K_{X, S}^{\circlearrowright} ' -->
<!-- \end{equation} -->

<!-- The leading eigenvectors $\tilde{P}$ and eigenvalues $\tilde{\Gamma}$ can then be obtained through the eigendecomposition of $\tilde{K}$. -->

<!-- Moreover, the policy covariance matrix $\Sigma_{S, S}$ is obtained as the inner product of the crowdsourced matrix $\tilde{K}_{X, S}^{\circlearrowright}$ with itself. -->

<!-- \begin{equation} -->
<!-- \Sigma_{S, S} = K_{X, S}^{\circlearrowright} ' K_{X, S}^{\circlearrowright} -->
<!-- \end{equation} -->

<!-- The eigendecomposition of $\Sigma_{S,S}$ is then -->

<!-- \begin{equation} -->
<!-- \Sigma_{S, S} = B \tilde{\Gamma} B' -->
<!-- \end{equation} -->

<!-- That is to say, the eigenvectors of the policy covariance matrix are just the orthonormal rotation matrix $B$. (See Schiek [-@] for details.) -->
...
[
-this is automatic anyway since the correlation matrix is deduced from $m$ loadings
The variable-PC correlations $K_{X, S}$ are sometimes referred to as the "loadings"
-loadings
-varimax rotation
-policy covariance matrix]

In the context of the portfolio optimization method presented above, this implies that each solution on the optimal menu is associated with a particular strategic objective.
...
# Deducing values...in minimum data contexts
...
group_knfo <- list(list_groups, group_names, group_colors)
...
# plot_corrXS_barchart(mat_Lrot, group_knfo, xAxis_title, sigNames)
# 
# ggsave("Fig 6.tiff")
...
group_fn <- function(group_knfo){
  list_groups <- group_knfo[[1]]
  group_names <- group_knfo[[2]]
  group_colors <- group_knfo[[3]]
...
get_hist_dfs_ready <- function(mat_ts_kn, group_knfo){
  #------------------------------------------------------------
  # Group the vars according to provided group info
  #varNames_ordered <- do.call(c, group_knfo[[1]])
  outlist <- group_fn(group_knfo)
  cols_ordered_by_group <- outlist[[1]]
  group_color_vec <- outlist[[2]]
  group_vec_ordered <- outlist[[3]]
...
optimize_portfolio <- function(M_knv, covmat_knv, covmat, mat_nab, targ_vec,
  ...
  #----------------------------------------------------
  if(!is.null(backtest_knfo)){
    nab_decRet_test <- backtest_knfo[["nab_decRet_test"]]
    covmat_test <- backtest_knfo[["covmat_test"]]
...
get_optimal_frontier <- function(covmat, mat_nab,
  ...
  if(is.null(fun_env)){
    ...
    backtest_knfo = fun_env[["backtest_knfo"]]
...
R_range <- 0.05
backtest_knfo <- list()
backtest_knfo[["nab_decRet_test"]] <- nab_decRet_test
backtest_knfo[["covmat_test"]] <- covmat_test
#--------------------------------------------------------------
fun_env_getOptFront <- list()
fun_env_getOptFront[["n_points_on_frontier"]] <- n_points_on_frontier
fun_env_getOptFront[["R_range"]] <- R_range
fun_env_getOptFront[["backtest_knfo"]] <- backtest_knfo
