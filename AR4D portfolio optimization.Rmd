---
title: Multi-objective, risk-adjusted optimization of agricultural research for development portfolios
author:
...
That is to say, $\mu_i$ can be mapped to polar coordinates, with the time horizon set to $2 \pi$, and $\omega  =  r T / 2 \pi$. Over the range $0 \geq t \geq 2 \pi$, this is a log spiral (Fig. ).
...
gg <- gg + annotate(
    ...
    label = expression(mu(theta)==x(theta)~e^{omega*(2~pi-theta)}), hjust = -0.1, vjust = -0.1, size = 3.5
  )
gg <- gg + annotate(
...
gg <- gg + annotate(
    geom = "text", x = thisX, y = thisY, 
    label = expression(mu(theta)==x(theta)~e^{(omega+i)*(2~pi-theta)}), hjust = -0.1, vjust = -0.1, size = 3.5
  )
gg <- gg + annotate(
    geom = "text", x = -0.1, y = 1.7, 
    label = "iy", size = 4
