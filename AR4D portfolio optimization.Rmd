...
The proposed workflow is partly inspired by the portfolio optimization method originally devised in the financial context [markowitz, merton], but differs from that convention in several key respects. Firstly, in the financial context expected returns scale linearly with investment, whereas in the AR4D context returns are marginally diminishing with respect to investment. Program NPV must therefore be defined to capture this reality.

Secondly, in the financial context, negative budget allocations are permissible, since they may be interpreted as short selling. In the AR4D context, on the other hand, a negative budget allocation has no clear interpretation. A valid AR4D portfolio optimization method must therefore guarantee strictly positive values in the optimal budget allocation. In practice, investors in the financial context also typically prefer portfolios with strictly long positions, and have devised various tricks for ensuring positive allocations. Here I present a novel method.
...
The budget constraint can be arbitrarily enforced 
...
# A novel formulation of the risk-adjusted AR4D portfolio optimization problem [replacing cost and return constraints with a vis viva constraint]

Based on the groundwork laid above, the risk adjusted portfolio optimization problem can now be formulated as follows.

[$n$ portfolio items]

[Comment on nabla_mu vs. nabla_w]
Because $\mu_i(w_i)$ is monotonically increasing in $w_i$, the 
...
&= \sqrt{\boldsymbol{\mu} ' G \boldsymbol{\mu}} - \frac{\lambda}{2} r^2 \boldsymbol{\mu} ' \boldsymbol{\mu} - \overline{\mathcal{E}})
\end{split}
\end{equation}
...
\begin{equation}
\nabla_{\mu} \mathcal{L} = \frac{1}{\sigma} G \boldsymbol{\mu} - \lambda r^2 \boldsymbol{\mu} = \mathbf{0}
\end{equation}

The optimal program NPV vector $\boldsymbol{\mu}^*$ is thus given by any of the eigenvectors of $G$ scaled by $1 / \sigma$, with corresponding vis viva shadow price $\lambda$ given by the respective eigenvalues, scaled by $r^2$. Letting $Q \Gamma Q'$ represent the spectral decomposition of $G$, with $\mathbf{q}_j$ and $\gamma_j$ standing for the $j^{th}$ column of $Q$ and its corresponding eigenvalue in $\Gamma$, the $j^{th}$ solution can be written

\begin{equation}
\boldsymbol{\mu}_j^* = \frac{1}{\sigma} \mathbf{q}_j
\end{equation}

with corresponding $\lambda_j$

\begin{equation}
\lambda_j = r^2 \gamma_j
\end{equation}

The $j^{th}$ budget allocation satisfying the first order conditions is then
...
The budget constraint is arbitrarily enforced because eigenvectors are defined up to scaling.
