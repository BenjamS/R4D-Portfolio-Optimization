...
Where the shorthand $s_{kj} = \text{cov}[\Delta x_k/x_k(\hat{t}), \Delta x_j/x_j(\hat{t})]$ has been introduced. Note that this may be rearranged into a natural extension of the coefficient of variation $c_{kj}$, as follows.
...
This is a project NPV "coefficient of covariation", which may be interpreted as an indicator of the correlation between the NPVs of projects $k$ and $j$, analogous to the correlation coefficient. (Whereas the correlation coefficient is the covariance normalized by the product of the respective standard deviations, the coefficient of covariation is the covariance normalized by the product of the respective means.)
...
Where $G$ is the project NPV "coefficient of covariation matrix", i.e. the matrix whose elements are $c_{kj}$, where $c_{kk} = c_k^2$. The matrix $G$ is just the project NPV covariance matrix left and right multiplied by a diagonal matrix of the inverse project NPV means.
...
Project NPV given budget $C$ ($\tilde{x}_k(0)$) can be estimated at the start of the funding cycle based on ex-ante impact assessment. Given values for $\alpha_k$, $r$, $T_k$, and the budget allocation $w_k$, it is then possible to calculate $\mu_k(0)$ by Eq \ref{eq:muFnw}.
...
While it may be difficult to develop an intuitive grasp of the value of the project volatility parameter $s_k$, research managers (and/or stakeholders and/or the foresight economists conducting the program appraisal) generally have some idea of the uncertainty surrounding estimates of project NPV, such that they can be asked to quantify minimum or maximum bounds on the expected arithmetic return $r$. For example, they can usually make statements to the effect of: "In the very best of cases, the maximum arithmetic return to project $k$ by the end of the project is probably about $1.5$". (Or they may give the same information in percentage terms, or in terms of magnitude, which is then easily converted to arithmetic return.)

first have to convert to quarterly return!
This maximum arithmetic return may then be interpreted as the upper bound of the $95\%$ confidence interval about $r$. Let the upper and lower bounds of this interval be denoted $\bar{r}_k$ and $\underline{r}_k$, respectively. Since project NPV is assumed to follow a gBm, the arithmetic return is thereby assumed to be normally distributed and hence symmetric about $r$, such that the upper and lower bounds of the confidence interval are related as follows.

\begin{equation}
\frac{\underline{r}_k + \bar{r}_k}{2} = r
\end{equation}

The lower bound of the confidence interval is thus easily deduced from the elicited upper bound $\bar{r}_k$. Now, recalling the expression for the log return of a gBm (Eq ...), note that the arithmetic and log return are related as follows.
...
The upper and lower log returns (call these $\bar{\ell}_k$ and $\underline{\ell}_k$) over the judicious time step $\Delta t$ can thus be deduced from $\bar{r}_k$ and $\underline{r}_k$.
...
Where $z = 1.96$ is the standard score corresponding to the $95\%$ confidence interval of a normally distributed random variable. The standard score can of course be adjusted to match the practitioner's idea of a suitable confidence interval.

Because the arithmetic return is normally distributed, the 




Subtracting the second equation from the first and rearranging then gives an expression for the volatility parameter $s_k$ in terms of known parameters.


such that the upper and lower bounds of the confidence interval are related is symmetric about the mean; and hence the lower bound of the confidence interval $\underline{r}_k$ must be $r-1.5$.




Alternatively, an analogous statement regarding the minimum percentage return can likewise be elicited. ; or "in the very worst of cases, the minimum arithmetic return to project $k$ is probably about $-50\%$". Maybe it's good to do both.
Either way, the volatility $s_k$ can be deduced from the elicited information as follows. 

<!-- (Or they may give the same information in terms of magnitude, which is then easily transformed into percentage terms.) -->

 

These upper and lower error margins bracketing  Let these upper and lower bounds . Now, recall the expression for the log return of a gBm (Eq \ref{}) to see that the expected arithmetic and log return of a project NPV $x_k$ are related as follows.

\begin{equation}
\begin{split}
r = E \left[ \frac{\Delta x_k}{x_k} \right] = \frac{x_k(t + \Delta t) - x_k(t)}{x_k(t)} = \frac{x_k(t + \Delta t)}{x_k(t)} - 1} \\
\Rightarrow 
\end{split}
\end{equation}
<!-- https://quant.stackexchange.com/questions/47537/convert-arithmetic-returns-to-log-returns -->
<!-- (call this $\mathscr{R}_k$), which may then be converted to a log return ($\ell_k = \ln(x_k(T) / x_k(\hat{t}))$) by the formula. Now, since $\tilde{x}_k(t)$ is assumed to follow a geometric Brownian motion, the log return may also be expressed -->
<!-- about the expected percentage change in project NPV between the start and end of the project. Another phrase for "percentage change" is "arithmetic return" -->

\begin{equation}
\ell_k = \left(r - \frac{s_k^2}{2} \right) \tau_k + s_k \epsilon \sqrt{\tau_k}
\end{equation}
...
\begin{equation}
\begin{split}
\mu_k = E[\ell] \bigr|_{t = \hat{t}} &= \left(r - \frac{s_k^2}{2} \right) \tau_k \\
\sigma_k^2 = Var[\ell] \bigr|_{t = \hat{t}} &= s_k^2 \tau_k
\end{split}
\end{equation}
...
Where the elements of the vector $\mathbf{s}$ are the project volatility parameters $s_k$ deduced in Eq \ref{eq:sk}, and the elements of $\boldsymbol{\tau}$ are the project time horizons $\tau_k$.

Schiek's hypothetical example of project-policy correlations [-@] is reproduced in Fig ....
...
Interpreting policies as orthonormally rotated principal components of the (unobserved) returns $\Delta x_k / x_k$, the outer product of the elicited matrix $L^{\circlearrowright}$ with itself gives an approximate correlation If policies are The matrix of project correlations.


the covariance matrix of returns $\Delta x_k / x_k$ is first deduced from expert knowledge or opinion.
An approximate project correlation matrix $\tilde{K}_{x, x}$ can be deduced from expert opinion using a method developed  The approximate covariance matrix $\tilde{\Sigma}_{x,x}$  and coefficient of covariation matrix $\tilde{G}_{x,x}$ can then be constructed as follows.
...
\end{split}
\end{equation}
<!-- Such that a phase correcting time horizon adjustment is automatically triggered if $\mu_k(0)$ is negative or complex valued at the start of the funding cycle. This redefinition of the project time horizon is particularly important for avoiding complex valued budget allocations.  -->
<!-- But, if the modified definition of $T_k$ in Eq \ref{eq:Tk} is introduced, then it is evident that $w_k$ is always real valued and positive. -->
<!-- In other words, a negative valued $\mu_k(0)$ is out of phase with the funding cycle by one half cycle. -->
<!-- More generally, -->
<!-- \begin{equation} -->
<!-- \mu_k(0) = x_k(0) e^{T_k (\omega + i) + i \rho_k \pi} -->
<!-- \end{equation} -->
...
<!-- It follows that projects with $\rho_k > 0$ can be brought into phase by extending or shortening the time horizon by $\rho_k \pi$ of a cycle. To see this, replace $T_k$ with $T_k - \rho_k \pi$. -->
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \mu_k(0) &= x_k(0) e^{(T_k - \rho_k \pi) (\omega + i) + i \rho_k \pi} \\ -->
<!-- &= x_k(0) e^{2 \pi \left(\nu_k - \frac{1}{2} \rho_k \right) (\omega + i) + i \rho_k \pi} \\ -->
<!-- &= x_k(0) e^{2 \pi \omega \left(\nu_k - \frac{1}{2} \rho_k \right)} e^{2 \pi i \left(\nu_k - \frac{1}{2} \rho_k \right) + i \rho_k \pi} \\ -->
<!-- &= x_k(0) e^{2 \pi \omega \left(\nu_k - \frac{1}{2} \rho_k \right)} e^{2 \pi i \nu_k} e^{- \rho_k \pi i + \rho_k \pi i} \\ -->
<!-- &= x_k(0) e^{2 \pi \omega \left(\nu_k - \frac{1}{2} \rho_k \right)} -->
<!-- \end{equation} -->
...
<!--  If extending, then $\kappa_k$ is set to $1$. If contracting, then $\kappa_k = 0$. Either way, this horizon adjustment brings the project into phase with the funding cycle, such that $\mu_k(0) > 0$. If expected project NPV is positive to begin with (and thus already in phase), then $\kappa_k$ defaults to $1 / 2$, such that no phase adjustment is made. -->
<!-- \begin{equation} -->
<!-- \begin{split} -->
...
```{r, fig.show = 'hold', fig.width=4, fig.height=2, fig.align='left', fig.cap="\\label{fig:rotCorrXSnParams}(Left) Bar chart of hypothetical project-policy correlations, (right) table of hypothetical project parameters."}
...
confInt95 <- paste(yMin, yMax, sep = "/")
# Deduce s
zBound <- 1.96
#sVec <- (yMax - yMin) / (2 * zBound * sqrt(TvecTstep))
sVec <- (yMax - yMin) / (2 * zBound)
sVec <- round(sVec, 3)
#cvVec <- round(sqrt(exp(sVec^2 * TvecTstep) - 1), 3)
#-----
projNames <- df_table$Project
df_param <- data.frame(proj = projNames, alpha, xTilde,
                       Tt, confInt95, c = cvVec)
df_param <- df_param %>% gather(var, val, alpha:c)
...
names(facetTitles) <- unique(df_param$var)
#   as_labeller(c(`alpha` =  "Scale\nelasticity",
# `c` = "Coef. of variation",
# `confInt95` = "Arithmetic return\n95% conf. interval",
# `Tt` = "Time horizon\n(Years/Quarters/Fund. Cyc.)",
# `xTilde` = expression(tilde(x))))
...
#eMatProj <- eigen(cormat_XX_derived)$vectors[, 1:nSig]

GxxTilde <- exp(SigdxdxTilde) - 1
gammaGxx <- eigen(GxxTilde)$values[1:nSig]
QxxTilde <- eigen(GxxTilde)$vectors[, 1:nSig]

SigdSdS <- t(mat_Lrot) %*% mat_Lrot
qP <- colSums(QxxTilde)
SigSS <- diag(qP) %*% (exp(SigdSdS) - 1) %*% diag(qP)
# diag((mat_Lrot) %*% t(mat_Lrot))
gammaSigSS <- eigen(SigSS)$values
eVecSigSS <- eigen(SigSS)$vectors
...
mat_muStar <- QxxTilde
TkHat <- 2 * pi * TvecFcyc
mat_wShareStar <- matrix(NA, nProj, nSig)
...
#colSums(mat_wShareStar)
visVivMOvec <- round(visVivMOvec, 4)
muStarMOvec <- colSums(mat_muStarMO)
sigVecMO <- round(gammaSigSS, 4)
betaStarMOvec <- round(betaStarMOvec, 4)
row.names(mat_muStarMO) <- projNames
colnames(mat_muStarMO) <- as.character(sigVec)
row.names(mat_wShareStarMO) <- projNames
colnames(mat_wShareStarMO) <- as.character(sigVec)
#--------------------------------------------------------------------
