...
\begin{equation}
\mu_k(0) = \tilde{x}_k(0) \left( \frac{w_k}{C} \right)^{\alpha_k} e^{r T}
\label{eq:muFnw}
\end{equation}
<!-- ... -->
...
# The cyclically real reality of project NPV
...
## Project NPV is cyclically real \label{sec:cycReal}
...
In other words, the value of the investment during the transformative process is not real, at least not in any immediately tangible or demonstrable sense. One may speak of "project value" at any point in the funding cycle, but one understands that this refers not to any palpable value the project might have at that point, but rather _either_ the value of the monetary investment at the start of the cycle, _or_ the expected value of project deliverables at the end of the cycle.

In this sense, project NPV is "cyclically real". That is, it is cyclically tangible or demonstrable, particularly at the start and end of funding cycles. One may also discern a sunk cost point, where the value is real but negative. At other points in the cycle, project NPV exists, but not in a way that intersects with our reality in any tangible or demonstrable sense.

Note that the same can be said of complex numbers $e^{i \theta}\;;\;\; i = \sqrt{-1},\;\;\theta \in (0, 2 \pi)$. That is, complex numbers are cyclically real---at $\theta = 0$, $\theta = \pi$, and $\theta = 2 \pi$. Elsewhere, they exist, but have no physically meaningful interpretation (forget about quantum mechanics for now). The complex plane thus presents us with a more realistic framework by which to model the cyclically real nature of project NPV. This means replacing $\mu_k(\hat{t})$ (equation ...) as a model of expected project NPV with $\mu_k(\hat{\theta})$, defined as follows.

Euler's formula... key identities
...
\begin{equation}
\mu_k(\hat{\theta}) = x_k(\hat{\theta}) e^{(2 \nu_k \pi - \hat{\theta})(\omega + i) }
\end{equation}

Where the duration of the funding cycle $T$ is replaced by $2 \pi$.

At the start of the funding cycle,

\begin{equation}
\mu_k(0) = x_k(0) e^{2 \nu_k \pi (\omega + i)}
\end{equation}

Which by Euler's identity ($e^{2 \nu_k i \pi} = 1$) reduces to
...
\begin{equation}
\begin{split}
\mu_k(2 \pi) &= x_k(2 \pi) e^{2 \pi (\nu_k - 1) (\omega + i)} \\
&= x_k(2 \pi) e^{2 \pi (\nu_k - 1) \omega}
\end{split}
\end{equation}

Since the two models (Eq ... and Eq ...) must give the same project NPV at the start of the funding cycle, $r$, $T$, and $\omega$ can be expressed in terms of each other by setting the two models equal at $\hat{t} = \hat{\theta} = 0$.

\begin{equation}
\begin{split}
x_k(0) e^{2 \nu_k \pi(\omega + i) } &= x_k(0) e^{r T \nu_k} \\
e^{2 \nu_k \pi \omega} &= e^{r T \nu_k} \\
\Rightarrow \omega &= \frac{r T}{2 \pi}
\end{split}
\end{equation}
...
\end{equation}
 ...
 (Recall that $e^{2\kappa i \pi} = 1$ for any integer $\kappa$.) This may then be extended to all angles $\theta \in (0, 2 \pi)$ as follows

\begin{equation}
\mu_k(\theta; T, r) = x_k(\theta) e^{(2 \pi - \theta) (\omega + i)}
\end{equation}

In this formulation, program value $\mu_k(\theta; T, r)$ is real-valued at the start of the funding cycle ($\theta = 0$), midway through ($\theta = \pi$), and at the end of the cycle ($\theta = 2 \pi$); and is complex-valued elsewhere. Real program values thus correspond to the points in the funding cycle when the investment has a tangible or demonstrable form---particularly at the beginning, when the funds are disbursed, and at the end, when the investment has been productively consumed and transformed into measurable program impacts. The negative real value occurring halfway through the cycle, at $\theta = \pi$, may be interpreted as the point in the funding cycle when the investment is demonstrable as a sunk cost. Complex values lying off the real number line, meanwhile, correspond to points in the funding cycle when the investment has no demonstrable or tangible form, i.e. when it is in the process of being transformed from its original monetary form into the planned project impacts.

## Defining project time horizons in terms of funding cycles

By definition, then, a funding cycle begins and ends on a real value. The corresponding project time horizon, $T_k$, it follows, may be expressed as an integer multiple of the funding cycle $T$.
...
\end{cases}
\end{equation}
<!-- This, of course, implies that the definition of program NPV also be modified as follows. -->
<!-- \begin{equation} -->
<!-- \mu_k(0) = \tilde{x}_k(0) \left(\frac{w_k}{C} \right)^\alpha_k (-1)^{2 \kappa_k + 1} e^{rT \left(\kappa_k + \frac{1}{2} \right)} \label{eq:muFnwKap} -->
<!-- \end{equation} -->
...
The budget allocations satisfying the first order conditions ($w_{kj}^*$) are given by Eq \ref{eq:wFnmuKap} evaluated at $\mu_{kj}^*$.

\begin{equation}
\mathbf{w}_{kj}^* = C \left(\frac{q_{kj}}{\tilde{x_k(0)}} (-1)^{2 \kappa_k + 1} e^{-r T \left( \kappa_k + \frac{1}{2} \right)} \right)^{\frac{1}{\alpha_k}}
\label{eq:wStar}
\end{equation}

Generally speaking, the eigenvectors $\mathbf{q}_j$ may include negative elements, implying negative expected program NPV ($\mu_{kj}(0)^* < 0$). As explained in \ref{sec:cycReal}, negative program NPV may be interpreted as the NPV evaluated at the sunk cost point of the program funding cycle. For the purposes of portfolio optimization, this is problematic, as it implies complex valued budget allocations. However, as explained above, the budget allocation can be made real valued in such cases by extending or shortening the program's time horizon by an odd integer multiple of one half the funding cycle. This is achieved by setting $\kappa_k$ to an integer value.
...
\begin{equation}
\mathbf{w}_{kj}^* = C \left(\frac{\beta_j q_{kj}}{\tilde{x_k(0)}} (-1)^{2 \kappa_k + 1} e^{-r T \left( \kappa_k + \frac{1}{2} \right)} \right)^{\frac{1}{\alpha_k}}
\label{eq:wStarPolicy}
\end{equation}
<!-- \begin{equation} -->
