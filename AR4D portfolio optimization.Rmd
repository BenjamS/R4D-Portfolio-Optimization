...
Where the shorthand $s_{kj}$ has been introduced to stand for the covariance between the quarterly arithmetic returns of projects $k$ and $j$. That is, $s_{kj} = \text{cov}[\Delta x_k/x_k(\hat{t}), \Delta x_j/x_j(\hat{t})]$, with $s_{kk} = s_k^2$. Note that Eq \ref{eq:xCov} can be rearranged into an extension of the coefficient of variation as follows.
...
&= e^{D(\boldsymbol{\tau}) \Sigma_{XX} D(\boldsymbol{\tau})} - 1 \;\;;\;\;\; [\Sigma_{XX}]_{kj} = s_{kj}
\end{split}
\label{eq:G}
\end{equation}

Where the elements of the vector $\boldsymbol{\tau}$ are the project time horizons $\tau_k$, and $\Sigma_{XX}$ is the quarterly arithmetic return covariance matrix.
...
An approximate project quarterly returns covariance matrix $\tilde{\Sigma}_{XX}$ then follows as

\begin{equation}
\tilde{\Sigma}_{XX} = D(\mathbf{s}) \tilde{K}_{XX} D(\mathbf{s})  \label{eq:SigXX}
\end{equation}

(Where the elements of the vector $\mathbf{s}$ are the project volatility parameters $s_k$ deduced in Eq \ref{eq:sk}.) Having deduced $\tilde{\Sigma}_{XX}$, an approximate coefficient of covariation matrix $\tilde{G}$ then follows from Eq \ref{eq:G}.
...
The number of retained eigenvectors $m$ should be high enough to explain, for example, $\zeta_m \geq 0.95$, although the exact number is left to the practitioner's discretion.

## A menu of solutions catering to a range of risk appetites

The "optimal" solution ($\boldsymbol{\mu}_1^*$) is optimal in the sense that it maximizes expected portfolio impact (vis viva) subject to a risk constraint. However, this is also the solution that corresponds to the lowest portfolio risk ($\min(\boldsymbol{\sigma}^*)$). It may be that the other candidate solutions are higher risk, but also higher reward. ...the optimal thus near the last of the trailing eigenvectors, but because scaled by $\beta_j^2$ not necessarily the last. 
...
Should make decision based on the vis viva to risk ratio
...
Finally, political considerations likely come into play. Solutions that allocate most of the budget to just one or two projects are bound to create a toxic work environment. ... adjusting time horizons to avoid this
...
In the illustrative example below, a donor uses the method described in the sections above to optimize an investment across the hypothetical AR4D project portfolio referenced in Fig \ref{fig:egProjCorr}. The menu of $m$ eligible optimal budget allocations is calculated using Eq \ref{eq:wStarVec}.

To make this calculation, the donor requires the global parameters $C$ (the budget) and $r$ (the quarterly discount rate). Additionally, for each project, the donor requires the time horizon $T_k$, the current maximum NPV $\tilde{x}_k$ and scale elasticity $\alpha_k$ (revisit section \sec{sec:dimRet} for details), and the standard deviation of the quarterly arithmetic return $s_k$. The $m$ leading eigenvectors of the project NPV coefficient of covariation matrix $G$ are also required.
...
For ease of interpretation, the budget is normalized to $100$. An annual discount rate of `r rYrlyArith` is assumed, which works out to a quarterly discount rate of `r round(rQrtrlyArith, 4)`. The current maximum project NPV $\tilde{x}_k$ is usually reported in ex-ante impact assessments, or can be extrapolated from the information given in such assessments. Estimation of the scale elasticity parameter $\alpha_k$ is not currently a standard part of AR4D ex-ante impact assessments, but can be reasonably elicited through consultation with experts and literature. Given sufficient prior notice, both $\tilde{x}_k$ and $\alpha_k$ could, in principle, be estimated econometrically, using, for example, data gathered during field trials. The hypothetical values for $\tilde{x}_k$ and $\alpha_k$ used in this example are displayed in the left panel of Fig \ref{fig:params}.

As discussed in section \ref{sec:cycReal}, project time horizons must be integer multiples of the duration of the funding cycle. In this example, the duration of the funding cycle is 3 years. Hypothetical time horizons are assigned to each project in the central panel of Fig \ref{fig:params}, displayed in terms of funding cycles, years, and quarters. Projects are assumed to be in varying stages of implementation at the time of the present allocation exercise, and so the displayed time horizons do not necessarily indicate the projects' full duration.

The standard deviation of quarterly arithmetic return $s_k \sqrt{\tau_k}$ is deduced from the upper or lower bound of the 95% confidence interval about the expected arithmetic life of project (LoP) return. The upper or lower bound itself can be deduced from ex-ante impact assessments and/or experts and literature. The hypothetical elicited LoP return 95% confidence interval, implied quarterly return 95% confidence interval, and values of $s_k$ deduced therefrom, are presented in the right panel of Fig \ref{tab:params}.
...
rTkArith_max <- ErTkArith + exp(rnorm(nProj, 0.35, 0.65))
...
df_sDeduce <- data.frame(Project = projNames, confInt95LoP, confInt95qrtrly, sVecQrtrly, sVecLoP)
cNames_sDeduce <- c("95% Conf. Int.\nLoP Arithmetic Return\n(Elicited)", "95% Conf. Int.\nQuarterly\nArithmetic Return\n(Implied)", "Stand. Dev. of\nLoP return\n(Deduced)", "Stand. Dev. of\nQuarterly Return\n(Deduced)")
...
gg_alphaXtilde + gg_t + gg_sDeduce + plot_layout(widths = c(1, 1, 2))
...
For purposes of demonstration, the portfolio of projects described in Fig \ref{fig:params} is deliberately diverse in research themes, impact pathways, expected rewards, risk levels, time horizons, and stages of implementation. Real AR4D portfolios may or may not be so heterogeneous.

To deduce the coefficient of covariation matrix $G$, an approximate project quarterly returns correlation matrix $\tilde{K}_{XX}$ is first "reverse engineered" from expert opinion and/or literature using Schiek's method [-@]. This is the matrix displayed in the right panel of Fig \ref{fig:egProjCorr} (revisit Eq \ref{eq:KXX} for details). The corresponding covariance matrix $\tilde{\Sigma}_{XX}$ then follows from $\tilde{K}_{XX}$ and the deduced $s_k$. An approximate coefficient of covariation matrix $\tilde{G}$ then follows from $\tilde{\Sigma}_{XX}$ and the time horizons $T_k$ (revisit Eqs \ref{eq:SigXX} and \ref{eq:G} for details). The matrix $\tilde{G}$ is displayed in the top panel of Fig \ref{fig:Gtilde}. The individual and cumulative portions of portfolio covariation described by each of the eigenvectors of $\tilde{G}$ are displayed in the bottom panel. The `r nSigG` leading eigenvectors of $\tilde{G}$, accounting for `r 100 * thresh`% of portfolio variance (marked by the dashed line in the figure), are retained as eligible solutions.
...
#------------------------------------------------------------------
# Plot Gtilde
fig_title <- "Approximate AR4D project NPV coefficient of covariation matrix"
colnames(Gtilde) <- projNames
row.names(Gtilde) <- projNames
ggGtilde <- plot_covmat(Gtilde, fig_title, round_to = 4, graph_on = F)
#------------------------------------------------------------------
...
Solving for $\beta_j$ using a root finding algorithm. And time adjustments
...
visVivVec <- round(visVivVec, 4)
muStarVec <- colSums(mat_muStar)
riskVec <- round(sqrt(upsTilde) * betaStarVec, 4)
visRiskRatio <-  round(r^2 / 2 * abs(betaStar) / sqrt(upsTilde), 4) #visVivVec / riskVec
#riskPriceVec <- round(r^2 / riskVec, 7)
#betaStarVec <- round(betaStarVec, 4)
row.names(mat_muStar) <- projNames
solCnames <- paste0("Solution ", c(1:nSigG), "\n", riskVec, " / ", visRiskRatio)
...
df_plot <- transform(df_plot,
                     Policy = factor(Policy, levels = gathercols))
colnames(df_plot)[ncol(df_plot)] <- "Budget share"
df_plot <- merge(df_plot, df_match_group, by = "Project")
df_plot <- df_plot[order(df_plot$Group), ]
df_plot$Project <- factor(df_plot$Project, levels = unique(df_plot$Project))
