...
abstract: |
  Conventional agricultural research for development project (or program) priority setting procedures lack a portfolio level tool by which to transform project impact assessments and/or rankings into funding allocations. As a result, funding decisions lack transparency and are vulnerable to subjective bias. The conventional funding allocation procedure also takes little account of project risk, and much less of project covariances, both of which are necessary to calculate portfolio risk. As a result, research portfolios are generally unbalanced and quite risky. To redress these issues, here I develop a risk-adjusted portfolio optimization tool to determine the impact maximizing research budget allocation across a portfolio of research alternatives with varying expected impacts, risk, and time horizons, subject to a portfolio risk constraint.
keywords: 
  - keyword1
  - keyword2
...
Mills clearly identified and articulated the need for such a tool long ago [-@Mills1998], and yet work in this direction has not yet begun in earnest. Instead of portfolio optimization, AR4D institutions hold "priority setting exercises", which Mills breaks down into 4 steps, as follows [-@Mills1998].
...
The persistent absence of Mill's fifth step has left AR4D funding decision-making mechanisms vulnerable to institutional inertia, stakeholder politics, and other forms of subjective bias [@Birner2016; @mccalla2014cgiar]. The Consultative Group on International Agricultural Research (CGIAR), in particular, is said to "have a long history of good intentions but limited success in developing appropriate approaches for priority setting" [@Birner2016]. The historic effort begun in 2011 to restructure CGIAR financing around a research program (CRP) portfolio model proved emblematic of this "limited success", when the results of an extensive priority setting exercise were summarily rejected by the board of directors in favor of a highly subjective, opaque, and ad hoc resource allocation [@Birner2016]. Earlier attempts at reform likewise devolved into a political melee [@mccalla2014cgiar]; and the current reform effort towards a "One CGIAR", begun in 2019, is headed in the same direction at the time of writing.
<!-- Such recurring episodes are symptomatic of a longstanding, deeply entrenched distrust of budget allocation procedures among research communities, across disciplines. [which is increasingly aggravated by steadily deteriorating fiscal outlooks.] Attempts to align funding with impact are condemned as "development at the expense of research" [@Birner2016], or even the "Balkanization" of research [@Petsko2011]. "One of the geniuses" of the CGIAR was once said to be its ability to consistently attract "aid funding of long-term research" while keeping "aid professionals from setting research agendas" [@mccalla2014cgiar]. At odds with the pro-poor mandate...at many points in history has proven to be responsive to demand and criticism. ...it is because of the lack of a mechanism...credible commitment problem -->

More importantly, the classical 4 step method of priority setting articulated by Mills takes little or no account of portfolio risk. Emeritus scientists at the International Center for Tropical Agriculture recall implementing an informal technique to balance research risk against expected impact when allocating funds in the early years of the Center's existence [@JCock_perscomm; @lynam2017forever]; but the technique was never formalized, and subsequently fell into disuse. Alston and Norton acknowledged in 1995 that the treatment of risk in impact assessment models "is rudimentary and in need of further refinement" [-@Alston1995]; but work has not yet begun on this "refinement". The estimation of project covariances---or synergies and tradeoffs, if you like---which is also necessary for the calculation of portfolio risk, has received even less attention (none, to my knowledge). Research portfolios funded on the basis of the classical four step priority setting method are thus, de facto, severely unbalanced, and hence potentially quite risky.
...
The proposed method solves for the impact maximizing budget allocation subject to a risk constraint, and is valid for portfolios of projects whose respective net present values 1) evolve as a geometric Brownian movement (gBm), and 2) exhibit marginally diminishing returns to investment. Moreover, the proposed method rests on three novel methodological propositions:
...
The proposed workflow is partly inspired by the portfolio optimization method originally devised in the financial context [markowitz, merton], but differs from that convention in a few key respects, especially as regards the methodological novelties just mentioned above. In passing, it is interesting to note that, empirically, financial "eigenportfolios" (i.e., portfolios balanced in accordance with one of the financial covariance matrix eigenvectors) perform relatively well, even though such budget allocations are generally not solutions to the classical financial portfolio optimization problem []. Efforts to devise a theoretical explanation for the empirical success of eigenportfolios are ongoing []. The present work is effectively one such explanation, and may thus be viewed as a contribution to that conversation.

Efforts are also underway to guarantee strictly positive budget allocations based upon the eigenportfolio approach. (Negative budget shares have a valid interpretation in the financial context in terms of short selling, but are problematic for other reasons [].) However, these efforts focus on extensions of the Perron-Frobenius theorem [see, for example, @...; @], and thus apply only to the leading eigenvector. This is problematic because---as I show below---the leading eigenvector is associated with the risk-maximizing budget allocation, while the trailing (retained) eigenvector is optimal. The method of guaranteeing positive budget shares proposed here, via rotations in the complex plane, applies to all eigenvectors.
...
# Project params
C <- 100 #Budget
alpha <- c(0.65, 0.65, 0.6, 0.75, 0.65, 0.6, 0.65, 0.8, 0.7, 0.75, 0.7) + 0.1
#xTilde <- C * exp(rnorm(nProj, 1.1, 0.3))
xTilde <- C * exp(rnorm(nProj, 1.1, 0.7))
...
Of these, only the $\hat{n}^{th}$ allocation satisfies the second order condition, and is thus optimal. The optimal portfolio vis viva is thus `r optVis` dollars squared per annual quarter squared (or project-joules), with a risk (standard deviation) of `r optRisk`, and thus a vis viva-to-risk ratio of `r optRatio`. A closer look at the optimal budget allocation is presented in Fig ... given in both percentage and dollar terms for a budget of USD $100 million.

```{r, fig.show = "hold", fig.width = 2, fig.height=2, fig.align="center", fig.cap="\\label{fig:wStarMenu}Menu of eligible optimal budget allocations.", echo = FALSE}

df_plot <- data.frame(Percent = round(mat_wShareStar[, nSigG], 2),
                      USD = mat_wShareStar[, nSigG] * 10^6)
gathercols <- colnames(df_plot)
df_plot$Project <- projNames
...
thisTitle <- "Budget: $100 million over 3 years"
...
AR4D project ranking and budget allocation procedures tend to be opaque and politically driven...leaving project funding decisions vulnerable to politics, institutional inertia, and other forms of subjective bias. The Consultative Group on International Agricultural Research, in particular, is said to "have a long history of good intentions but limited success in developing appropriate approaches for priority setting" [@Birner2016]. In the sections above I have attempted to remove the methodological premise for this "limited success" by introducing a risk adjusted AR4D portfolio optimization method to determine optimal budget allocations.
