...
In this article, I propose a risk-adjusted portfolio optimization method to redress these issues. The proposed method solves for the risk minimizing allocation of funds, subject to a level of effort constraint, across a portfolio of research projects (or programs), each of which may have varying expected impacts, scaleability, time horizons, risk, synergies, and tradeoffs. The method is built upon the assumptions that 1) project net present values evolve as a geometric Brownian motion and 2) scale with investment, but exhibit marginally diminishing returns to investment. The work presented here also rests on three novel methodological propositions:
...
\begin{equation}
u_k(t; t + \Delta t) = E[\Delta x_k] = \mu_k(t; t + \Delta t) - x_k(t)
\end{equation}

By analytical continuation, this may be extended to any time horizon $T_k$.

\begin{equation}
u_k(t; T_k) = E[x_k(T_k) - x_k(t)] = \mu_k(t; T_k) - x_k(t)
\end{equation}
...
<!-- \frac{\rho}{2} \text{v}^2 =  -->
\begin{equation}
\mathcal{E} = \rho \frac{r^2}{2} \boldsymbol{\mu} ' \boldsymbol{\mu}
\label{eq:ErawPort}
\end{equation}
