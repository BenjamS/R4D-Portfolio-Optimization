...
\begin{equation}
\begin{split}
\nabla_{\boldsymbol{\mu}}^2 \mathcal{L} &= r^2 I - \frac{r^2}{\sigma} G \\
\Rightarrow \nabla_{\boldsymbol{\mu}}^2 \mathcal{L}|_{\boldsymbol{\mu}^*} &= r^2 I - \frac{\lambda_j}{\sigma^*_j} G \boldsymbol \\
&= r^2 I - \frac{r^2}{\beta_j^2 \upsilon_j} G 
\end{split}
\end{equation}

Substituting the right hand side of this equation for $\nabla_{\boldsymbol{\mu}}^2 \mathcal{L}|_{\boldsymbol{\mu}^*}$ in the previous equation then gives

\begin{equation}
\begin{split}
H_j &= D(\boldsymbol{\beta}_j) Q_j ' (r^2 \boldsymbol{\mu}_j^* - \frac{\lambda_j}{\sigma} G \boldsymbol{\mu}_j^*) Q_j D(\boldsymbol{\beta}_j) \\
&= D(\boldsymbol{\beta}_j) Q_j ' (r^2 \beta_j \mathbf{q}_j - \frac{r^2}{\beta_j^2 \upsilon_j} G \mathbf{q}_j) Q_j D(\boldsymbol{\beta}_j) \\
&= r^2 (I - \frac{1}{\beta_j^2 \upsilon_j} D(\boldsymbol{\upsilon}_j))
\end{split}
\end{equation}

Where $\boldsymbol{\upsilon}_j$ is the vector of eigenvalues of $G$ excluding the $j^{th}$ eigenvalue. 
