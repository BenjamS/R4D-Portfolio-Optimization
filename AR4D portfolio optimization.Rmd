...
\begin{equation}
\mathbf{w}_{\ell}^* = C (D(\tilde{\mathbf{x}}(0))^{-1} D(e^{-r \mathbf{T}}) \beta_{j \ell}^* \boldsymbol{\mu}_{\ell}^* )^{\frac{1}{\boldsymbol{\alpha}}}
\label{eq:wStarPolicy}
\end{equation}

<!-- To avoid complex valued elements in $\boldsymbol{\beta}_{\ell}^*$, recall that negative elements in $\mathbf{b}_j$ can be made positive by adjusting the program time horizon, as explained in section \ref{sec:cycReal}. -->
