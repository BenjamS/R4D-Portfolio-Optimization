...
<!-- In some contexts, it may make sense to use a sigmoidal ansatz that exhibits increasing returns to investment up to a point of constant returns, and decreasing returns thereafter---such as $x_k(0) = \tilde{x}_k(0) e^{-C/w_k}$. -->
<!-- ... -->
...
\begin{equation}
\frac{\Delta u}{\Delta t} \bigr|_{t = T} > \frac{\Delta u}{\Delta t} \bigr|_{t = 0}
\end{equation}
...
\begin{equation}
v = \frac{\Delta u}{\Delta t}\bigr|_{*} - \frac{\Delta u}{\Delta t}
\end{equation}
...
\begin{equation}
\Delta u_k \bigr|_{t = \hat{t}} = \mu_k(\hat{t}) = x_k(\hat{t}) e^{r \tau_k} \bigr|_{t = \hat{t}}
\end{equation}
...
For ease of interpretation, the budget is normalized to 100, and the project maximum NPVs are expressed in the same units. An annual discount rate of `r rYrlyArith` is assumed, which works out to a quarterly discount rate of `r round(rQrtrlyArith, 4)`. The current maximum project NPV $\tilde{x}_k$ is usually reported in ex-ante impact assessments, or can be extrapolated from the information given in such assessments. Estimation of the scale elasticity parameter $\alpha_k$ is not currently a standard part of AR4D ex-ante impact assessments, but can be reasonably elicited through consultation with experts and literature. For commodity projects, given sufficient prior notice, both $\tilde{x}_k$ and $\alpha_k$ could, in principle, be estimated econometrically, using, for example, data gathered during field trials. Hypothetical values for $\tilde{x}_k$ and $\alpha_k$ are assigned to each project in the left panel of Fig \ref{fig:params}.
...
df_plot <- df_plot[order(df_plot$Group), ]
df_plot$Project <- factor(df_plot$Project, levels = unique(df_plot$Project))
theseCnames <- c("Million USD per\nFunding Cycle", "Percent\nof Budget", "Time horizon\n(Funding Cycles)")
...
The portfolio of projects appearing in the hypothetical use case examined above is diverse in research themes, impact pathways, expected rewards, risk levels, time horizons, and stages of implementation. Real AR4D portfolios may or may not be so heterogeneous. In some decision-making contexts, the assumption that projects scale with investment may be too strong, in which case the practitioner might consider applying the method at a more aggregate level, say over a portfolio of programs, each consisting of numerous projects and complementary activities in various stages of implementation. Applications are not limited to project portfolios. AR4D breeding programs might consider using the method to optimize allocation of resources across a portfolio of genotypes, for example.

In the hypothetical example above, the optimal (risk-minimizing) solution is also the solution with the highest expected impact (vis viva). However, this is not necessarily always so. In cases where the risk minimizing solution does not align with the impact maximizing solution, ambitious investors might consider choosing one of the sub-optimal budget allocations with higher expected impact in exchange for higher risk. The benefits of any particular portfolio must also be weighed against how evenly distributed the allocation is. A solution allocating 60% or more of the budget to just one or two projects is likely to generate toxic discontent in the research community.
