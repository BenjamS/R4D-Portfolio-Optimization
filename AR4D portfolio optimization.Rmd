...
df_alphaXtilde <- data.frame(proj = projNames, alpha, xTilde)
cNames_alphaXtilde <- c(expression("Scale\nelasticity - "*alpha*""), expression("Expected\nmax net benefit\n(millions of USD) - "*tilde(x)*""))
df_t <- data.frame(proj = projNames, Years = TvecYrs, Quarters = TvecTstep, FundCyc = TvecFcyc)
cNames_t <- c("Funding\nCycles", "Quarters", "Years")
df_sDeduce <- data.frame(proj = projNames, confInt95LoP, confInt95qrtrly, sVecQrtrly, sVecLoP)
...
gg <- ggplot(df_plot, aes(x = var, y = proj,
                           label = val))
