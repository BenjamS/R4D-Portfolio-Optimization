...
# eiThetHat <- diag(exp(-TkHat * omega)) %*% diag(1 / xTilde) %*% muHatStar
# thetHat <- acos(Real(eiThetHat))
# rhoKpi <- thetHat - TkHat
# ind <- which(Ptilde < 0)
# PtildeStar <- Ptilde
# PtildeStar[ind] <- - exp(- omega * pi) * PtildeStar[ind]
# QtildeStar <- diag(cvVec) %*% PtildeStar
# rho_k <- asin(Im(QtildeStar))
# Tk <- 2 * pi * (TvecFcyc + rho_k / 2)
# mat_woCstar <- (diag(exp(-Tk * (omega + 1i))) %*% diag(1 / xTilde) %*% QtildeStar)^(1 / alpha)
...
matWgts <- sqrt(matWgtsSq + 0i)
QtildeStarMO <- diag(cvVec) %*% Ptilde %*% matWgts
# ind <- which(PtildeStarMO < 0)
# PtildeStarMO[ind] <- - exp(- r * tFundCyc / 2) * PtildeStarMO[ind]
rho_k <- asin(Im(QtildeStarMO))
Tk <- 2 * pi * (TvecFcyc + sign(rho_k) * rho_k / 2)
woCstarMO <- (diag(exp(-Tk * (omega + 1i))) %*% diag(1 / xTilde) %*% QTildeStarMO)^(1 / alpha)
