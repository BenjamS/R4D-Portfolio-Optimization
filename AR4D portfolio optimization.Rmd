...
Given $s_k$ and $\mu_k$, it is then straightforward to compute the variances $\sigma_k^2$ and coefficients of covariation $c_k$.

An approximate project correlation matrix $\tilde{K}_{X, X}$ can be deduced from expert opinion using a method I develop elsewhere [-@]. The approximate covariance matrix $\tilde{\Sigma}_{X,X}$  and coefficient of covariation matrix $\tilde{G}_{X,X}$ can then be constructed as follows.
...
# The cyclically real reality of expected project NPV \label{sec:cycReal}
...
## Negative expected NPV and phase corrected budget allocation
...
To bring the project into phase with the funding cycle, the time horizon can be extended or shortened by half a funding cycle, i.e. by $\pi$.
...
Phase correction is particularly useful to avoid complex valued budget allocations. An expression for the project budget allocation is obtained by solving Eq \ref{eq:muFnwComp} for $w_k$ at $\theta = 0$.
...
Whereby it is evident that $w_k$ is complex valued if $\mu_k(0) < 0$ and $T_k = T \nu_k$. But, if the modified definition of $T_k$ in Eq \ref{eq:Tk} is introduced, then it is evident that $w_k$ is always real valued.
...
\begin{equation}
\mathbf{w}_{kj}^* = C \left(\frac{q_{kj}}{\tilde{x_k(0)}}e^{- T_k (\omega + i)} \right)^{\frac{1}{\alpha_k}}
\label{eq:wStar}
\end{equation}
<!-- \begin{equation} -->
...
\begin{equation}
\mathbf{w}_{kj}^* = C \left(\frac{\beta_j q_{kj}}{\tilde{x_k(0)}} e^{-T_k (\omega + i)} \right)^{\frac{1}{\alpha_k}}
\label{eq:wStarPolicy}
\end{equation}
<!-- \begin{equation} -->
...
\begin{equation}
\begin{split}
\boldsymbol{\mu}^* &= \Sigma_{j = 1}^m \boldsymbol{\mu}_j^{**} \\
&= \Sigma_{j = 1}^m \mathbf{q}_j \beta_j^*
\end{split}
\end{equation}

Where the optimal scaling parameters $\beta_j^*$ are defined as those which maximize policy impact subject to a policy risk tolerance. Formally,
...
\begin{equation}
\begin{split}
\mathcal{E}^p &= \Sigma_{j = 1}^m \mathcal{E}_j^* = \frac{1}{2} \Sigma_j \mathbf{v}_j^* ' \mathbf{v}_j^* \\
&= \frac{r^2}{2} \Sigma_j \boldsymbol{\mu}_j^* ' \boldsymbol{\mu}_j^* = \frac{r^2}{2} \boldsymbol{\beta} ' \boldsymbol{\beta} \\
\sigma^p &= \mathbf{1} ' \Sigma_{p,p} \mathbf{1} = \boldsymbol{\mu}^p ' G_{p,p} \boldsymbol{\mu}^p \\
&= \boldsymbol{\beta} ' G_{p,p} \boldsymbol{\beta}
\end{split}
\end{equation}

Where $G_{p,p}$ is the policy coefficient of covariation matrix, and where the control variables $\boldsymbol{\mu}^p$ are the expected policy NPVs, whose elements are defined
...
The matrix $G_{p,p}$ can be deduced from the policy NPV covariance matrix $\Sigma_{p,p}$, which in turn is obtained as the inner product of the project-policy correlation matrix (Eq \ref{eq:Lrot}) with itself.

\begin{equation}
\Sigma_{p,p} = K_{X, S}^{\circlearrowright} ' K_{X, S}^{\circlearrowright}
\end{equation}
