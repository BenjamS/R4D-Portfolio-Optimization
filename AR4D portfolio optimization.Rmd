...
The lagrangian multiplier $\lambda$ may be interpreted as the risk shadow price, that is to say, the marginal cost of a marginal increment in risk above the tolerance threshold $\overline{\sigma}$. Formally,
...
By equation \ref{eq:muFnw}, the solutions $\boldsymbol{\mu}_j^*$ can then be solved for the optimal budget allocations as follows.
...
Where $\beta_j$ is an arbitrary scaling constant, which may be chosen such that $\Sigma_k w_k = C$.
...
The "optimal" solution ($\boldsymbol{\mu}_1^*$) is optimal in the sense that it maximizes expected portfolio impact subject to a risk constraint. However, this is the solution that also corresponds to the highest eigenvalue ($\gamma_1$), and is hence also the riskiest solution (recall equation \ref{eq:gammaRisk}). The other candidate solutions ($\boldsymbol{\mu}_1^*, \boldsymbol{\mu}_2^*,\dots, \boldsymbol{\mu}_n^*$) may correspond to lower expected impacts, but also successively decreasing eigenvalues ($\gamma_2, \gamma_3, \dots, \gamma_n$), and hence successively decreasing risk levels.
...
[### AR4D correlation matrix eigenvectors encode AR4D policies]

Generally speaking, each of the $m$ leading eigenvectors of a covariance matrix of a complex system of variables encode information about a specific aspect of the system. In the financial context, Gopikrishnan et al. [-@gopik2001quant] show that the each of the leading eigenvectors of a covariance matrix of hundreds of stock price series is associated with movements in a particular industry or sector. And each element in an eigenvector reflects a particular stock's contribution to, or participation in, movements in the respective sector.
<!-- [The authors even suggest this method might be used to classify firms into industries.] -->

In technical language, this is because empirical collective movements (for e.g., sectorial movements) tend to align with the principal components of the system. Eigenvectors reflect the participation of individual variables (for e.g., stocks) in a particular collective movement because they are very nearly the covariances of variables ($X$) with principal components ($S$). The exact covariances of variables with principal components ($\Sigma_{X, S}$) is given by

\begin{equation}
\Sigma_{X, S} = P \Gamma
\end{equation}

Correlations between variables and principal components ($K_{X, S}$), it follows, are given by 

\begin{equation}
K_{X, S} = \tilde{P} \tilde{\Gamma}^{\frac{1}{2}}
\end{equation}

(See Schiek [-@schiek2021reverse] and/or Abdi [-@] for details.)

Elsewhere, I extend the logic of Gopikrishnan et al.'s analysis to AR4D correlation matrices, such that the collective movements associated with each eigenvector may be interpreted as the AR4D institution's (and/or donor's) strategic objectives or policies [-@schiek2021reverse]. The elements of the eigenvectors, it follows, may be interpreted as reflecting each research project's (or program's) contribution towards the respective policy.
...
<!-- \begin{equation} -->
<!-- E[x_k(T)]\bigr|_{t = 0} = \mu_k = x_k(0) e^{\omega 2 \pi} -->
<!-- \label{eq:ExTrad} -->
<!-- \end{equation} -->

<!-- That is to say, $\mu_k$ can be mapped to polar coordinates, with the time horizon set to $2 \pi$, and $\omega  =  r T / 2 \pi$. Over the range $0 \geq \theta \geq 2 \pi$, $\mu_k$ traces out one cycle of a log spiral (Fig. ). -->


<!-- Secondly, note that this has an equivalent expression in the complex plane. -->

<!-- \begin{equation} -->
<!-- \mu_k = -x_k(0) e^{2 \pi (\omega + i)} -->
<!-- \label{eq:ExTcomp} -->
<!-- \end{equation} -->
...
<!-- Finally, consider the case where $\mu_k$ is negative. That is, say $\mu_k = -\zeta$ where $\zeta > 0$. -->

<!-- \begin{equation} -->
<!-- -\zeta = -x_k(0) e^{2 \pi (\omega + i)} -->
<!-- \end{equation} -->

<!-- By Euler's formula ($e^{(2 n - 1)i \pi} = -1 \:;\:\:\:n\in \mathbb{Z}$), this can be rewritten -->

<!-- \begin{equation} -->
<!-- e^{ i \pi} \zeta = -x_k(0) e^{2 \pi (\omega + i)} -->
<!-- \end{equation} -->

<!-- Dividing through by $e^{i \pi}$ then gives -->

<!-- \begin{equation} -->
<!-- \zeta = -x_k(0) e^{\omega 2 \pi + i} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \zeta = x_k(0) e^{\omega \pi} -->
<!-- \end{equation} -->

<!-- Which then maps back to the original form as -->

<!-- \begin{equation} -->
<!-- \zeta = x_k(0) e^{r \frac{T}{2}} -->
<!-- \end{equation} -->
