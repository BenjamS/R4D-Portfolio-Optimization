...
# The cyclically real reality of expected project NPV
...
Where the funding cycle $T$ is mapped to $2 \pi$, so that $\omega 2 \pi = r T$; and the time step $\Delta t$ maps to $\Delta \theta$ such that $2 \pi \Delta \theta = T\Delta t$. Now, consider that, by Eq \ref{eq:compNum}, $e^{2i \pi} = 1$. Hence, $\mu_k(0)$ may also be written
...
&= \tilde{x}_k(\theta) \left(\frac{w_k}{C} \right)^{\alpha_k} e^{(2 \nu_k \pi - \theta)(\omega + i)}
\label{eq:muFnwComp}
\end{equation}
<!-- [graphic? $\mu_k(\hat{\theta}) / x_k(\hat{\theta})$ decay---ratio of E[x(T)]|_t / x(t)] -->

## Negative expected NPV and phase correction

This extension of expected project NPV to the complex plane encompasses the sunk cost point occurring halfway through the funding cycle ($\theta = \pi$), when the investment is real valued, but negative.
...
\begin{split}
\mu_k(0) &= -x_k(0) e^{2 \nu_k \pi \omega} \\
&= x_k(0) e^{2 \nu_k \pi \omega \pm i \pi}
\end{split}
\end{equation}

To bring the project into phase with the funding cycle, the time horizon can be extended or shortened by $\pi$.

\begin{equation}
\begin{split}
\mu_k(0) &= -x_k(0) e^{(2 \nu_k \pi \pm \pi) (\omega + i)} \\
&= -x_k(0) e^{(2 \nu_k \pi \pm \pi) \omega} e^{(2 \nu_k \pm 1) i \pi} \\
&= x_k(0) e^{2 \pi \omega \left(\nu_k \pm \frac{1}{2} \right) \omega}
\end{split}
\end{equation}
...
\begin{equation}
\mathbf{w}_{kj}^* = C \left(\frac{q_{kj}}{\tilde{x_k(0)}}e^{-r T_k} \right)^{\frac{1}{\alpha_k}}
\label{eq:wStar}
\end{equation}
<!-- \begin{equation} -->
