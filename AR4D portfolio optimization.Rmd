...
wSlack <- function(beta, Tk, omega, xTilde, muStar, alpha){
  wShare <- ((diag(exp(-Tk * omega)) %*% diag(1 / xTilde) %*% muStar) * beta)^(1 / alpha)
  wSlack <- sum(wShare) - 1
  return(wSlack)
}
thisInt <- c(0, 100)
for(j in 1:nSig){
  muHatStar <- mat_muHatStar[, j]
  ind <- which(muHatStar < 0)
  rhoK <- rep(0, length(TkHat))
  rhoK[ind] <- 1
  Tk <- TkHat - rhoK * pi
  muStar <- muHatStar
  muStar[ind] <- -exp(- omega * pi) * muStar[ind]
  ...
  betaStar <- out$root
  print(out$f.root)
  wShareStar <- ((diag(exp(-Tk * omega)) %*% diag(1 / xTilde) %*% muStar) * betaStar)^(1 / alpha)
