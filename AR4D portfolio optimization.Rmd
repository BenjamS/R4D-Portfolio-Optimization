...
The standard deviation of quarterly arithmetic return $s_k \sqrt{\tau_k}$ is deduced from the upper or lower bound of the 95% confidence interval about the expected arithmetic life of project (LoP) return (revisit section \ref{sec:volEst} for details). The upper or lower bound itself can be deduced from ex-ante impact assessments and/or experts and literature. The hypothetical elicited LoP return 95% confidence interval, implied quarterly return 95% confidence interval, and values of $s_k$ deduced therefrom, are presented in the right panel of Fig \ref{fig:params}.
...
To deduce the coefficient of covariation matrix $G$, an approximate project quarterly returns correlation matrix $\tilde{K}_{XX}$ is first "reverse engineered" from expert opinion and/or literature using Schiek's method [-@]. This is the matrix displayed in the right panel of Fig \ref{fig:egProjCorr} (revisit Eq \ref{eq:KXX} for details). The corresponding covariance matrix $\tilde{\Sigma}_{XX}$ then follows from $\tilde{K}_{XX}$ and the deduced $s_k$. An approximate coefficient of covariation matrix $\tilde{G}$ then follows from $\tilde{\Sigma}_{XX}$ and the time horizons $T_k$ (revisit Eqs \ref{eq:SigXX} and \ref{eq:G} for details). The leading eigenvector-eigenvalue pairs $\{\mathbf{q}_j,\upsilon_j\}$ of $\tilde{G}$ accounting for `r 100 * thresh`% of portfolio variance are retained as eligible solutions to the portfolio optimization problem. [In this example, that number works out to `r nSigG`]

The corresponding menu of optimal budget allocations $\mathbf{w}_j$ is then calculated up to scaling by Eq \ref{eq:wStar}. As explained in section \ref{sec:cycReal}, complex valued elements in $\mathbf{w}_j$ are transformed into real valued elements by extending or shortening the project time horizon by half a funding cycle. In this example, all such adjustments are extensions. The value of $\beta_j$ is chosen so as to enforce the budget constraint, that is, such that $\mathbf{w}_j {'} \mathbf{1} = C$ (revisit section \ref{sec:scaleConst} for details).
...
#------------------------------------------------------------------
# Plot Gtilde
fig_title <- "Optimized AR4D project NPV correlation matrix"
...
colnames(Kstar) <- projNames
row.names(Kstar) <- projNames
ggGtilde <- plot_covmat(Kstar, fig_title, round_to = 4, graph_on = F)
#------------------------------------------------------------------
...
* can also use for trait portfolio optimization
