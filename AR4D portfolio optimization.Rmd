...
\begin{equation}
\begin{split}
\boldsymbol{\mu}_j^* ' \nabla_{\mu} \mathcal{L} &= 2 r^2 \mathcal{E}_j - \lambda_j \sigma_j = 0 \\
&\Rightarrow \lambda_j = 2 r^2 \frac{\mathcal{E}_j}{\sigma_j}
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
2 r^2 \frac{\mathcal{E}_j}{\sigma_j} &= r^2 \frac{\sigma_j}{\gamma_j} \\
\Rightarrow \gamma_j &= \frac{\sigma_j^2}{2 \mathcal{E}_j}
\end{split}
\label{eq:gammaRisk}
\end{equation}
...
Correlations between variables and leading principal components ($\tilde{K}_{X, S}$), it follows, are given by 

\begin{equation}
\tilde{K}_{X, S} = \tilde{P} \tilde{\Gamma}^{\frac{1}{2}}
\end{equation}
...
And the policy optimization problem can be rewritten
...
The eigenvectors of $G_{S, S}$ may be deduced from the crowdsourced matrix $K_{X, S}^{\circlearrowright}$ as follows. First, recall that the policy covariance matrix $\Sigma_{S, S}$ can be obtained as the inner product $K_{X, S}^{\circlearrowright} {'} K_{X, S}^{\circlearrowright}$, and that the eigendecomposition of $\Sigma_{S, S}^{\circlearrowright}$ works out to
...
That is to say, the eigenvectors of \Sigma_{S, S}^{\circlearrowright} are just the orthonormal rotation matrix $B$, and the respective eigenvalues are just the $m$ leading eigenvalues of the program cross coefficient of variation matrix $G_{X, X}$ and program correlation matrix $K_{X, X}$. (See Schiek [-@schiek2021reverse] for details.) Since $G_{S, S} = D(\boldsymbol{\mu}^S)^{-1} \Sigma_{S, S} D(\boldsymbol{\mu}^S)^{-1}$ by definition, then the eigenvectors of $G_{S, S}$ are $D(\boldsymbol{\mu}^S)^{-1} B$
...
...it turns out the eigenvectors of $\Sigma_{S, S}$ are just the orthonormal rotation matrix $B$.
