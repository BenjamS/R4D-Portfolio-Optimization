...
In the illustrative example below, a donor uses the method described in the sections above to optimize an investment of USD $50 million across the hypothetical AR4D project portfolio referenced in Fig \ref{fig:egProjCorr}. The optimal investment in each project is calculated using Eq \ref{eq:wStarVec}.

To make this calculation, the donor requires, for each project, the time horizon $T_k$, the current maximum NPV $\tilde{x}_k$ and scale elasticity $\alpha_k$ (revisit section \sec{sec:dimRet} for details), and the standard deviation of the quarterly arithmetic return $s_k$. The leading eigenvectors of the project NPV coefficient of covariation matrix $G$ are also required.

The current maximum project NPV $\tilde{x}_k$ is usually reported in ex-ante impact assessments, or can be extrapolated from the information given in such assessments. Estimation of the scale elasticity parameter $\alpha_k$ is not currently a standard part of AR4D ex-ante impact assessments, but can be reasonably elicited through consultation with experts and literature. Given sufficient prior notice, both $\tilde{x}_k$ and $\alpha_k$ could, in principle, be estimated econometrically, using, for example, data gathered during field trials. The hypothetical values for $\tilde{x}_k$ and $\alpha_k$ used in this example are displayed in the left panel of Fig \ref{fig:params}.
...
cNames_alphaXtilde <- c(expression("Scale\nelasticity - "*alpha*""), expression("Expected\nmax net benefit\n(millions of USD) - "*tilde(x)*""))
df_t <- data.frame(Project = projNames, Years = TvecYrs, Quarters = TvecTstep, FundCyc = TvecFcyc)
...
To deduce the coefficient of covariation matrix $G$, an approximate project quarterly returns correlation matrix $\tilde{K}_{XX}$ is first "reverse engineered" from expert opinion and/or literature using Schiek's method [-@]. This is the matrix displayed in the right panel of Fig \ref{fig:egProjCorr} (revisit Eq \ref{eq:KXX} for details). The corresponding covariance matrix $\tilde{\Sigma}_{XX}$ then follows from $\tilde{K}_{XX}$ and the deduced $s_k$. An approximate coefficient of covariation matrix $\tilde{G}$ then follows from $\tilde{\Sigma}_{XX}$ and the time horizons $T_k$ (revisit Eqs \ref{eq:SigXX} and \ref{eq:G} for details). The 3 leading eigenvectors of $\tilde{G}$, accounting for  95% of portfolio variance, are retained as eligible solutions (Fig \ref{fig:varExplained}).
<!-- {sec:volEst} -->

**Fig 4. Plot of the individual and cumulative portions of portfolio variance** explained by each of the eigenvectors of the crowdsourced project coefficient of covariation matrix.
<!-- &nbsp; -->
<!-- The plot shows that, for the financial data set, the leading `r nSig` signals are sufficient to meet this criterion. -->
<!-- \ref{fig:wStarVecs} -->

```{r, fig.show = 'hold', fig.width=5, fig.height=2, fig.align='left', fig.cap="\\label{fig:varExplained}Plot of the individual and cumulative portions of portfolio variance explained by each of the eigenvectors of the crowdsourced project coefficient of covariation matrix.", echo=F}
...
Solving for $\beta_j$ using a root finding algorithm.

Up for rebalancing at the end of every funding cycle (3 years). i.e. the allocation is repeated with updated parameters.
...
df_plot <- merge(df_plot, df_match_group, by = "Project")
df_plot <- df_plot[order(df_plot$Group), ]
df_plot$Item <- factor(df_plot$Project, levels = unique(df_plot$Project))
