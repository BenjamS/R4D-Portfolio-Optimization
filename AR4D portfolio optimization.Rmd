...
\begin{equation}
\begin{split}
\sigma_j^* &= \mathbf{q}_j ' G \mathbf{q}_j \\
&= \upsilon_j
\end{split}
\end{equation}
...
&= Q_j ' (r^2 \mathbf{q}_j - \frac{r^2}{\upsilon_j} G \mathbf{q}_j) Q_j \\
&= r^2 (I - \frac{1}{\upsilon_j} U_j)
\end{split}
\end{equation}

Where $U_j$ is the diagonal matrix of eigenvalues of $G$ excluding the $j^{th}$ eigenvalue. Since $G$ is positive definite, its eigenvalues are positive. And since $\upsilon_1 > \upsilon_2 > ... > \upsilon_n$ for a portfolio of $n$ items, it is straightforward to see that $H_1$ is negative definite, $H_n$ is positive definite, and the matrices $H_{1 < k < n}$ are indefinite.
...
The percentage of portfolio variance explained by any given eigenvector $\mathbf{q}_j$ (call this $k_j$) is the corresponding eigenvalue $\upsilon_j$ divided by the sum of all the eigenvalues (See Abdi [-@] for details).
...
Nonetheless, selection of a high risk solution from the optimal menu must be justified by a correspondingly high expected impact (vis viva). Obviously, a lower risk solution with higher expected impact should be selected over a higher risk solution with relatively lower vis viva.

Finally, political considerations come into play. Solutions must be reasonably distributed among projects. Selection of a solution that allocates most of the budget to just one or two projects is bound to create a toxic work environment.
