...
Finally, note that the same analysis can be reformulated in terms of project NPV---that is to say, in terms of the difference between the net benefit stream with the project intervention versus the net benefit stream without it (the counter-factual). Formally, this means $v$ is redefined
...
<!-- ## A menu of solutions tailored to different levels of risk tolerance -->

* Only the risk maximizing solution has (a chance at) positive budget allocation.

* The eigenvectors of $\tilde{\Sigma}$ may be interpreted as policies [@apopkyshan, @schiek]. [* Orthonormal rotation] The inner product $L 'L$ is the policy covariance matrix.
...
The interpretation is nuanced. Minimizing cost is also in some sense minimizing return. Different levels of return corresponding to different levels of risk tolerance
As mentioned earlier, the vis viva reflects both portfolio cost and portfolio return. It is conceivable, then, that some investors may be interested in the intermediate portfolio solutions...or even the risk maximizing solution... The investor should choose the solution with the eigenvalue that best matches their level of risk tolerance. [corresponding to each solution indicates the risk per unit vis viva.]  [no need to have separate cost and return functions. Cost is return. (Sure?)]

<!-- Elsewhere I have shown how an orthonormal rotation can be applied to clarify each portfolio item's contribution to each policy.  -->
