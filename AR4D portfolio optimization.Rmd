...
\begin{equation}
\sigma(\hat{t})^2 = \boldsymbol{\mu}' G \boldsymbol{\mu} \end{equation}

Where $G$ is the "cross coefficient of variation matrix", i.e. the matrix whose diagonal elements are the squared coefficients of variation $c_k^2$, and whose off-diagonal elements are the cross coefficients of variation $c_{kj}$. Note that $G$ is just the covariance matrix left and right multiplied by a diagonal matrix of the inverse project means.
...
# Program value is cyclically real \label{sec:cycReal}
...
\begin{equation}
\mu_k(\theta; T, r) = x_k(\theta) e^{(\omega + i) (2 \pi - \theta)}
\end{equation}
...
### Interpreting the risk shadow price and eigenvalues

Note, in passing, that dotting the first order conditions through by $\boldsymbol{\mu}_j^*$ and solving for $\lambda_j$ results in an instructive expression for the risk shadow price.
...
\begin{equation}
\begin{split}
\boldsymbol{\mu}_j^* ' \nabla_{\mu} \mathcal{L} &= 2 \mathcal{E}_j - \lambda_j \sigma_j = 0 \\
&\Rightarrow \lambda_j = 2 \frac{\mathcal{E}_j}{\sigma_j}
\end{split}
\end{equation}

The risk shadow price may thus be thought of as a "return to risk ratio". More specifically, $\lambda_j$ is proportional to the ratio of the portfolio vis viva---a measure of expected portfolio impact---to the portfolio risk.

Note, moreover, that setting this expression equal to the previous one results in an instructive expression for the eigenvalues.

\begin{equation}
\begin{split}
2 \frac{\mathcal{E}_j}{\sigma_j} &= r^2 \frac{\sigma_j}{\gamma_j} \\
\Rightarrow \gamma_j &= r^2 \frac{\sigma_j^2}{2 \mathcal{E}_j} \\
&= \frac{\gamma_j}{\beta_j^2}
\end{split}
\label{eq:gammaRisk}
\end{equation}

(Where $\mathcal{E}_j$ is the expected portfolio impact associated with the $j^{th}$ solution.)

The $j^th$ eigenvalue of $G$ thus reflects the portfolio risk (squared) per unit expected impact for that solution. In this way, $\lambda_j$ and $\gamma_j$ are effectively indicators of the risk involved in choosing one solution over another.

### Optimal budget allocation

By equation \ref{eq:muFnw}, the candidate solutions $\boldsymbol{\mu}_j^*$ can then be solved for the optimal budget allocations as follows.
...
\begin{equation}
\begin{split}
\mathbf{w}_j^* &= C (D(\tilde{\mathbf{x}}(0))^{-1} \mathbf{q}_j)^{\frac{1}{\boldsymbol{\alpha}}} \\
&= C (D(\tilde{\mathbf{x}}(0))^{-1} D(\mathbf{c}) \mathbf{p}_j)^{\frac{1}{\boldsymbol{\alpha}}}
\label{eq:wStar}
\end{equation}
...
\begin{equation}
\mathbf{w}_j^* = C (D(\tilde{\mathbf{x}}(0))^{-1} \beta_j \mathbf{q}_j)^{\frac{1}{\boldsymbol{\alpha}}}
\label{eq:wStarScaled}
\end{equation}

Where $\beta_j$ is an arbitrary scaling constant, which may be chosen such that $\Sigma_k w_k = C$. More explicitly, then, the candidate program budget shares can be expressed

\begin{equation}
\begin{split}
w_{kj}^* &= C \left( \frac{\beta_j q_{kj}}{\tilde{x}(0)} \right)^{\frac{1}{\alpha_k}} \\
&= C \left( \frac{\beta_j c_k p_{kj}}{\tilde{x}(0)} \right)^{\frac{1}{\alpha_k}}
&= C \left( \frac{\beta_j \sqrt{e^{s_k^2} - 1} p_{kj}}{\tilde{x}(0)} \right)^{\frac{1}{\alpha_k}}
\end{split}
\end{equation}

[Note that this means $\mathcal{E}_j = r^2 \beta_j^2 / 2$, $\lambda_j = r^2 \beta_j^2 / \sigma_j = r^2 \beta_j^2 / \sqrt{\gamma_j}$ 

### Dealing with negative budget allocations

Generally speaking, the eigenvectors $\mathbf{q}_j$ may include negative values. As explained in \ref{sec:cycReal}, negative values may be interpreted as program funding cycles that are out of phase with the main funding cycle. Insofar as this interpretation is valid, negative weights can be made positive by extending or shortening that program's time horizon by an integer multiple of $2$. Mathematically, this means multiplying the solution $w_{kj}^*$ by $-e^{r T/(2 \kappa)}$, where $\kappa$ is an integer.
...
Alternatively, the investor may want to optimize across multiple strategic objectives. In other words, they may want a budget allocation that is an optimal linear combination of the portfolios on the optimal menu. This optimal linear combination can be determined by solving the portfolio problem stated in equation \ref{eq:probStat} at the policy level.

\begin{equation}
\max_{\boldsymbol{\mu}^S} \mathcal{E}^S \:\:\:\:s.t.\:\:\: \sigma^S = \bar{\sigma}^S
\end{equation}

Where $\mathcal{E}^S$ and $\sigma^S$ are the policy vis viva and risk, defined as the aggregation of the program level NPVs.

\begin{equation}
\begin{split}
\mathcal{E}^S &= \frac{1}{2} \Sigma_j \mathbf{v}_j^* ' \mathbf{v}_j^* \\
...
Where $G_{S,S}$ is the policy cross coefficient of variation matrix, and the control variables $\boldsymbol{\mu}^S$ are the expected policy NPVs. The elements of $\boldsymbol{\mu}^S$ are defined

\begin{equation}
\boldsymbol{\mu}_j^S = \boldsymbol{\mu}_j^* ' \mathbf{1}
\end{equation}
...
\begin{equation}
\boldsymbol{\mu}_j^S = \boldsymbol{\beta}
\end{equation}
...
\begin{equation}
\begin{split}
\mathcal{E}^S &= \frac{r^2}{2} \Sigma_j \boldsymbol{\beta} ' \boldsymbol{\beta}
...
The candidate solution $\boldsymbol{\beta}^*$ may thus be any of the eigenvectors of $G_{S, S}$, with corresponding policy risk shadow price ($\lambda^S$) given in terms of the respective eigenvalue.

The eigenvectors and eigenvalues of $G_{S, S}$ may be deduced from the crowdsourced matrix $K_{X, S}^{\circlearrowright}$ as follows. First, recall from section ... that the policy covariance matrix $\Sigma_{S, S}$ can be obtained as the inner product $K_{X, S}^{\circlearrowright} {'} K_{X, S}^{\circlearrowright}$. Moreover, note that the eigendecomposition of $\Sigma_{S, S}^{\circlearrowright}$ works out to

\begin{equation}
\Sigma_{S, S}^{\circlearrowright} = B \tilde{\Gamma} B '
\end{equation}

That is to say, the matrix of eigenvectors of $\Sigma_{S, S}^{\circlearrowright}$ is just the orthonormal rotation matrix $B$ that is applied to clarify program-policy correlations in equation \ref{eq:rotL}; and the respective eigenvalues are just the $m$ leading eigenvalues of the program cross coefficient of variation matrix $G_{X, X}$ (or program correlation matrix $K_{X, X}$). (See Schiek [-@schiek2021reverse] for details.)

Since $G_{S, S} = D(\boldsymbol{\beta})^{-1} \Sigma_{S, S} D(\boldsymbol{\beta})^{-1}$ by definition, it follows that the eigenvalues of $G_{S, S}$ are likewise $\tilde{\Gamma}$. Combining this information with the first order conditions (equation \ref{eq:focS}), it follows that

\begin{equation}
\lambda^S_j = r^2 \frac{\sigma^S}{\gamma_j}
\end{equation}

[\lambda = \lambda^S ?]
[\sigma = \sigma^S ?]

Moreover, the first order conditions dotted through...

\begin{equation}
\lambda^S = 2 \frac{\mathcal{E}^S}{\sigma^S}
\end{equation}

and the eigenvectors of $G_{S, S}$ and \Sigma_{S, S}^{\circlearrowright} are related as follows. Letting $\boldsymbol{\varepsilon}_j$ and $\mathbf{b}_j$ stand for the $j^{th}$ eigenvectors of $G_{S, S}$ and $\Sigma_{S, S}^{\circlearrowright}$, respectively,
...
The $j^{th}$ optimal policy budget allocation $\boldsymbol{\beta}_j^*$ is thus given by

\begin{equation}
\boldsymbol{\beta}_j^* = \mathbf{b}_j^{\frac{1}{2}}
\end{equation}

The corresponding policy risk shadow price $\lambda^S$ can then be found by rearranging
