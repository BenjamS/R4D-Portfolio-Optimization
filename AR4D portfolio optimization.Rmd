...
This system of equations can then be rearranged into three expressions for the volatility parameter $s_k$ in terms of known parameters (i.e. in terms of the upper and lower log returns $\bar{\ell}, \underline{\ell}$, the discount rate $r$, the project time horizon $T_k$, and the standard score $z$).

\begin{equation}
\begin{split}
s_k &= \frac{z \pm \sqrt{z^2 - 1 / 2 (\bar{\ell} - r \tau_k)}}{ \sqrt{\tau_k}} \\
&= \frac{-z \pm \sqrt{z^2 - 1 / 2 (\underline{\ell} - r \tau_k)}}{ \sqrt{\tau_k}} \\
&= \sqrt{ r - \frac{\bar{\ell} + \underline{\ell}}{2 \tau_k}}
\end{split}
\end{equation}
...
\sigma^p &= \mathbf{1} ' \Sigma_{p,p} \mathbf{1} = \boldsymbol{\beta} ' G_{p,p} \boldsymbol{\beta}
\end{split}
\end{equation}

Where $G_{p,p}$ is the policy coefficient of covariation matrix
