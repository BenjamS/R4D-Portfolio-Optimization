...
library(pracma)
library(nleqslv)
# source('D:/OneDrive - CGIAR/Documents/minV_EnetUconstr.R')
# source('D:/OneDrive - CGIAR/Documents/coloredNoise.R')
source('~/minV_EnetUconstr.R')
source('~/coloredNoise.R')
#============================================
#N risky assets
#============================================
m_b <- c(-0.37, -0.45, -0.28, -0.63)
cv_b <- c(0.45, 0.31, 0.52, 0.23) * sign(m_b)
s_b <- cv_b * m_b
...
for(i in 1:ncol(W_e)){
  ...
  Vbe[i] <- as.numeric(t(1 / w_e) %*% S_b %*% (1 / w_e))
}
Risk_e <- sqrt(Vbe)
