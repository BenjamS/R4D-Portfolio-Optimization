...
In market or near market contexts where the principle of risk-neutral valuation may be invoked, $r$ is the risk-free rate of return. In the AR4D context... By definition, project NPV appreciates with time at the discount rate. Therefore the growth rates should be set to the discount rate $r$ ..., 
...
[Comment on how to interpret negative velocity]
...
\begin{equation}
\frac{\Delta u_i}{\Delta t} = -r e^{r \tau} \mathbf{x}(\hat{t}) ' \mathbf{1}
\end{equation}
...
The risk adjusted portfolio optimization problem can now be formulated replacing the budget constraint with a vis viva constraint.
...
This is a cost function to be minimized. Both terms in the equation are costs. The sign on the vis viva constraint is positive because it too, like risk, is a cost.

Note that this is the marginal cost of a marginal increment in the vis viva above the budget $\overline{\mathcal{E}}$. That is,

\begin{equation}
\frac{\partial \mathcal{L}}{\partial (\mathcal{E} - \overline{\mathcal{E}})} = \lambda
\end{equation}

Alternatively, $\lambda$ can be thought of as the marginal benefit of the vis viva saved by spending less than the budget.
...
The optimal project NPV vector $\boldsymbol{\mu}^*$ is thus given by any of the eigenvectors of $K$, with corresponding vis viva shadow price $\lambda$ given by the negative of the respective eigenvalues. Since the matrix $K$ is symmetric, then its leading eigenvector generally has, or can be modified to have, only positive entries [@noutsos2006perron]. [linear combination of eigenvectors also a solution?]
...
[* Can also get these results using the Euler condition]

## SOC

[# N-fund fund theorem ? --> dimensional reduction of problem to policy portfolio optimization?]
