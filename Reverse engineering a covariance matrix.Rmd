---
title: "Reverse engineering covariance matrices in minimum data environments"
author:
...
Agricultural research for development (AR4D) institutions tend to give careful consideration to the formulation of their strategic objectives (SOs), but very little, if any, consideration to the tradeoffs and synergies that may arise between SOs. This is dual to another problem: AR4D institutions often take great care to assess the potential impacts of individual research projects, but little consideration to tradeoffs and synergies between projects. Synergies between SOs or projects can enhance portfolio impact while tradeoffs can effectively annul it. A clear understanding of SO and project tradeoffs and synergies is therefore essential to the optimal leveraging of investments.
...
Here I propose a method for deducing a complete matrix of project synergies and tradeoffs---also known as a correlation matrix---from domain knowledge based on principal components analysis (PCA). Specifically, I propose an expert or stakeholder survey whereby each project's correlation with the institution's strategic objectives (SOs) is elicited. I then show that, if the SOs are interpreted as orthogonally rotated principal components of the institution's problem space, then both SO and project correlation matrices can be deduced from this domain knowledge. Positive and negative correlations may then be interpreted as project synergies and tradeoffs, respectively.

I first provide a proof of concept using data from financial markets; and then walk through an illustrative example of how the method might be applied in the AR4D context.
...
plot_covmat <- function(covmat, fig_title = "Covariance Matrix", round_to = 2, graph_on = T, legend_position = NULL){
...
Concrete meaning can be attributed to the otherwise abstract signals by examining the loadings---i.e. by examining how correlated the signals are with the individual price series. Signal 4, for example, appears to have something to do with price movements in the communications sector. Signal 3, meanwhile, is concerned with Utilities and Real Estate, and so might be called the "Housing & Urban Development Signal". The interpretation of Signals 1 and 2, on the other hand, is not so straightforward, since they are correlated with many portfolio items. In such cases, it is often useful to apply an orthogonal rotation to $L$ in order to clarify the picture. That is to say, instead of examining $L$, one examines $L_{\circlearrowleft}$.
...
# Approximating the data covariance matrix from the loadings matrix
...
```
# The signals correlation matrix and "signal ROI" {#sigCovRoiSection}

Note that the orthogonally rotated signals correlation matrix (call this $\Sigma_{SS}^{\circlearrowleft}$) can likewise be obtained from the rotated loadings via the inner product $L_{\circlearrowleft}'L_{\circlearrowleft}$. 

\begin{equation}
\begin{split}
\label{eq:crowdsource_SOcovmat}
...
The unrotated signal variances $\tilde{\Gamma}$ and the rotation matrix $B$ can also be recovered via an eigendecomposition of this matrix. Moreover, with $\tilde{\Gamma}$ and $B$ in hand, it is then possible to derive the implicit leading eigenvectors of the data covariance matrix ($\tilde{P}$).

\begin{equation}
\tilde{P} = D(\boldsymbol{\sigma}_X) L_{\circlearrowleft} B'\tilde{\Gamma}^{-1 / 2}
\label{eq:deducePcrowd}
\end{equation}

With $\tilde{P}$ in hand, it is then possible, via equation \ref{eq:sigs_def}, to project a given set of project ROI onto the signals, thereby introducing the notion of "signal ROI" or "strategic objective ROI".

# "Reverse engineering" covariance matrices from domain knowledge
<!-- Equations \ref{eq:covmat_SS} to \ref{eq:Lrot} -->
The foregoing implies that it is possible to work backwards from the orthogonally rotated loadings $L_{\circlearrowleft}$ and asset risk $\boldsymbol{\sigma}_X$, to arrive at an approximate data covariance matrix. In other words, instead of starting with a covariance matrix and working towards a definition of the principal components, one starts from well defined principal components, and works towards a definition of the covariance matrix.

In the AR4D context, an institution's SOs are comparable to a set of principal components describing 90% of the problem space that is of interest to the institution. Given a portfolio of projects, a survey of domain experts and/or stakeholders could be conducted to determine how correlated each project is with each SO. These correlations may then be interpreted as orthogonally rotated loadings $L_{\circlearrowleft}$ corresponding to an unobserved dataset measuring project impact.^[it is important to have a clear notion of this unobserved dataset. When two projects are said to be correlated, what exactly about the two projects is correlated? In many cases, project return on investment (ROI) may be the variable of interest. But other measures might also be considered, such as net present value or scaleability.] The project impact correlation matrix can then be calculated from this information using equation \ref{eq:cormat_from_Lrot}.
...
A list of hypothetical AR4D projects is presented in table \ref{tab:exampLoadings}. If project risk is not calculated, it may be elicited as part of the survey following the suggestion in section ... above. The projects are loosely grouped into four categories to facilitate interpretation, but there is no strict rule followed, and clearly some overlap, in the grouping.
...
The survey exercise concludes. The resulting crowdsourced project-SO correlations are then interpreted as the orthogonally rotated loadings matrix $L_{\circlearrowleft}$. The project correlation matrix is reverse engineered from them via equation \ref{eq:cormat_from_Lrot}, while the SO correlation matrix is reverse engineered via equation \ref{eq:crowdsource_SOcovmat}.^[Since these are correlation matrices, their diagonal elements must equal 1. When deduced from a set of $m < n$ loadings (whether crowd- or data-sourced), the diagonal elements will diverge from 1. In the correlation matrices below, then, I correct for this divergence by dividing through by the diagonal.]
...
cormat_SS_derived <- D_sdCorrect %*% covmat_SS_derived %*% D_sdCorrect
...
A covariance matrix reverse engineered from domain knowledge in the manner proposed above offers a perspective on otherwise unquantifiable project synergies and tradeoffs. The accuracy of this perspective depends on 1) how completely the chosen SOs capture the evolution of projects within the problem space (in the literal sense of equation \ref{eq:SO_ck}); and 2) the accuracy of the domain knowledge whence SO-project correlations are deduced. It is thus imporant to apply this method in contexts where there is a high degree of confidence in domain knowledge compensating a general lack of good data (i.e. a high $\nu$ ratio). The method is open to criticism insofar as the domain knowledge is skewed by institutional inertia, politicized thinking, and other sources of subjective bias. However, the alternative method of covariance estimation based on data does not necessarily have a comparative advantage in this department, as it is likewise subject to a host of different, but no less problematic, sources of bias and error.

Regardless of accuracy, the proposed method may be used as a crowdsourcing tool to build institutional consensus in areas where expert opinions differ or are lacking altogether. The method confronts experts and/or stakeholders with potentially unexpected logical implications of what they (think they) know about the problem space, and about the evolution of projects through that space.

The illustrative example given above focuses on AR4D priority setting, but the method presented here is applicable to any portfolio level planning context with a high $\nu$ ratio. The method might also be used in pre-breeding activities, for example, to deduce trait and variety correlation matrices. An example of such a crowdsourcing exercise is given in Figure \ref{fig:corrXS_tratVarty}, with the variety and trait correlation matrices reverse engineered from this information given in Figure \ref{fig:cormatTraitVartys}.

```{r, fig.show='hold', fig.width=5, fig.height=3, fig.align='center', fig.cap="\\label{fig:corrXS_tratVarty}Hypothetical example of crowdsourced varimax rotated trait-variety correlations. (These correlations are randomly generated.)", echo=F}
#---------------------------------------------
n_varieties <- 10
df_Lrot <- data.frame(`Bean variety` = paste("Variety", c(1:n_varieties)),
                      Palatability = rnorm(n_varieties),
                      `Iron content` = runif(n_varieties),
                      #`Vit. A content` = runif(n_varieties),
                      `Protein content` = runif(n_varieties),
                      `Fast maturing` = rnorm(n_varieties),
                      `Drought resistant` = rnorm(n_varieties))
...
```{r, fig.show = "hold", fig.width = 6, fig.height = 4, fig.align = "left", fig.cap = "\\label{fig:cormatTraitVartys}Variety and trait correlation matrices reverse engineered from the hypothetical trait-variety correlations."}

cormat_XX_derived <- mat_Lrot %*% t(mat_Lrot)
#cormat_XX_derived <- round(cormat_XX_derived, 2)
fig_title <- "Reverse engineered variety\ncorrelation matrix"
gg_covmat_varty <- plot_covmat(cormat_XX_derived, fig_title, round_to = 2, graph_on = F, legend_position = "bottom")

cormat_SS_derived <- t(mat_Lrot) %*% mat_Lrot
#cormat_SS_derived <- round(cormat_SS_derived, 2)
fig_title <- "Reverse engineered trait\ncorrelation matrix"
gg_covmat_trait <- plot_covmat(cormat_SS_derived, fig_title, round_to = 2, graph_on = F, legend_position = "bottom")
...
Some may be tempted to use the reverse engineered covariance matrix in a risk-adjusted portfolio optimization problem. For example, if expected portfolio utility is defined

\begin{equation}
E[U] = \E[mathbf{x}] {'} \ln(\mathbf{w})
\end{equation}

where $E[\mathbf{x}]$ is the vector of expected project ROIs and $\mathbf{w}$ is the vector of budget shares invested in each project, then portfolio risk or variance, it follows, is defined

\begin{equation}
Var[U] = \mathbf{w} {'} \tilde{\Sigma}_{XX} \mathbf{w}
\end{equation}

In the absence of data by which to calculate the project covariances in $\Sigma_{XX}$, an AR4D institution may want to substitute the reverse engineered covariance matrix $\tilde{\Sigma}_{XX}$, and then solve the problem

\begin{equation}
\max{E[U]}_{\mathbf{w}} \:\:\: s.t. \:\:\: Var[U] = v \:\:\: \mathbf{w} {'} \mathbf{1} = c
\end{equation}

where $v$ and $c$ are the institution's risk and budget constraints, respectively. However, when solving this problem for the optimal budget shares it becomes necessary at some point to invert the covariance matrix; whereas the reverse engineered covariance matrix has $n-m$ eigenvalues equal to zero, and is thus non-invertible. Careful consideration must also go into the formulation of the portfolio utility function so as to avoid negative budget shares, and to accurately reflect decreasing returns to investment in any given project.^[The naive formulation above is analogous to financial portfolio optimization, where negative investment shares are permitted (short-selling), and returns are linear in investment.]

On the other hand, it is possible to optimize criteria weights, i.e. the weight or emphasis given to each SO by the institution and/or stakeholders. Such weights are often assigned in a highly subjective and ad hoc manner. The method presented here suggests a more rigorous, objective approach. The reverse engineered SO covariance matrix (equation \ref{eq:crowdsource_SOcovmat}) is invertible, and can be used in an optimization problem such as that which is stated above. In the formulation of the objective function $E[U]$, project ROI is replaced with SO ROI, deduced from the crowdsourced loadings via equations \ref{eq:deducePcrowd} and \ref{eq:sigs_def}. Again, care must be taken to ensure positive weights and an accurate representation of the benefits of marginal increases in weights.^[One simple (and perhaps facile) way to ensure positive weights is to replace $\mathbf{w}$ with $\ln(\mathbf{w})$.]
